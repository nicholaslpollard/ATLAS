...
**Current as of 2026-09-10 (UTC). This README and `docs/roadmap.md` are the only
...
- **B35 observable parallel DEVELOPMENT replay performance package is STAGED / ACCEPTANCE PENDING.** The repaired serial replay was deliberately interrupted for an execution-only performance package after an operator snapshot showed **69 / 482 deterministic groups**, **8,556 / 59,768 source units (14.32%)** complete, with the current restart averaging about **660.6 source units/hour** and projecting roughly **77.5 hours** still remaining. PR #72 preserves every frozen scientific input, strategy/outcome rule, group identity, self-hash receipt, authorization/trial identity, source/split fingerprint, protected-data boundary, and final scientific fingerprint semantics while scheduling only independent deterministic groups concurrently. On the dedicated 6-core/12-thread, ~24 GiB ATLAS workstation, hardware-auto targets **5 worker processes x 2 DuckDB threads = 10 aggregate worker threads**, reserving two logical CPUs plus practical RAM headroom for Windows/RDP/coordinator work. Existing valid completed receipts remain authoritative and reusable. Before the optimized canonical replay may resume, the isolated real-data equivalence/performance verifier must recompute completed canonical groups through the actual ProcessPool path and require byte-identical JSONL SHA-256 plus scientific receipt-field equivalence. The hard performance objective is a full canonical replay below 24 hours where the hardware permits it, with an 8-12 hour stretch objective; **no runtime target may weaken scientific validation, data coverage, outcome quality, or authority controls.** Workstation equivalence benchmarks now show 5 workers x 2 DuckDB threads at **1,744.4 units/hour / 34.3 hours full**, 8 x 1 at **2,560.9 units/hour / 23.3 hours full**, and 10 x 1 at **2,676.8 units/hour / 22.3 hours full**, with exact JSONL SHA-256 equivalence at 5/5, 8/8, and 10/10 sampled groups. The small 8-to-10-worker gain identifies a shared data/processing bottleneck rather than a concurrency shortage. The subsequent single-Parquet-scan experiment preserved exact scientific equivalence at **10/10** sampled groups but regressed to **820.8 units/hour / 72.8 hours full / 60.4 hours remaining**, versus the proven 10 x 1 baseline **2,676.8 units/hour / 22.3 hours full / 18.5 hours remaining**. It is therefore **REJECTED** and the source loader is restored to the proven dual-DuckDB-scan path. The best accepted execution shape remains **10 workers x 1 DuckDB thread**; the next optimization target is Python/object/strategy processing plus narrower per-worker connection, calendar, cache, and data-layout reuse. Canonical SHA-256 verification, fail-closed validators, frozen strategy semantics, and the exact-equivalence gate remain mandatory. That worker-resource-reuse probe is now **ACCEPTED** on workstation evidence: the 10 x 1 exact-equivalence run passed **10/10** JSONL SHA-256 comparisons at **2,781.1 source units/hour**, projecting **21.5 hours full / 17.8 hours remaining**, a **3.9% throughput improvement** over the prior 2,676.8-unit/hour 10 x 1 baseline. The retained implementation reuses only process-local DuckDB/calendar infrastructure; source/checkpoint reads, canonical SHA-256 verification, the proven dual-query validation/materialization path, and scientific output bytes remain unchanged. The next staged execution-only probe removes the intermediate Pandas `to_dict(orient="records")` allocation in canonical-bar construction by using `itertuples`, while deliberately retaining full `CanonicalBar.model_validate()` for every minute bar; exact golden-output equivalence remains mandatory before acceptance.
- **B35/A36 v2 preregistration and the finite DEVELOPMENT replay implementation are FROZEN PRE-OUTCOME.** Repository acceptance is exact-head gated; acceptance itself does not open B35 outcomes. The original `atlas-b35-a36-conditional-evidence-v1-pre-outcome` fingerprint `7bfd1cfdd65e946d45caa99dd2a35a90d8b424cb82cad5941ad26cac51816c4c` is preserved as **superseded pre-outcome lineage**; no B35 outcomes were opened under it. Before any outcome access, the control review corrected two preregistration defects without changing B34: a B34 bar stamped 11:30 ET is information-safe at 11:31 ET, and a raw prior-close/current-open gap that crosses an overnight split is `UNAVAILABLE` for non-gap condition profiling rather than invalidating an otherwise same-session ORB/premarket/HVD signal. The active contract is `atlas-b35-a36-conditional-evidence-v2-pre-outcome-clock-split-corrected` with fingerprint `145cc8983439b6062fd5e60303539d1611cdf7db0119ed68a7da1e68ffd8eda6`; B34 remains bound unchanged at `6f7239fcda11ac6c890d2635980d431ec49e346707d9cc549f111bd50daaa4bf`. The final signal-time bucket is `1031_TO_1131`, and 20-return realized volatility requires 21 completed closes in one split epoch.
- **The B35 finite DEVELOPMENT replay implementation is deliberately restartable, compact, and authority-limited.** It binds exact expected minute units to the immutable native acquisition-plan `year/month/batch/unit` identity, confines checkpoint/canonical/split artifacts to the isolated V2 roots without symlink escape, lazily verifies canonical SHA-256 before open, validates raw/unadjusted physical rows and strict Boolean adjustment schema, and rejects a mutated in-memory plan whose content no longer matches its fingerprint. A self-hash DEVELOPMENT authorization is published under an exclusive claim before outcomes; replay output is serialized, a self-hash outcome-read-start marker survives a failed authorized replay, completed symbol-batch outputs use self-hash receipts and restart-safe orphan handling, and the final run binds validated receipt identities. Entry delay is measured from the information-safe decision, exact target opens resolve at the target before later intrabar ambiguity, short returns/costs use entry notional, and entered-but-unresolved opportunities retain their attempted exposure evidence. The faster fired-setup scan only prefilters candidate bars and delegates final signal truth to the unchanged B34 evaluators. No giant permanent minute-feature lake is created. PIT market-regime context remains `UNAVAILABLE` when an exact accepted prior-session join is unavailable rather than guessed. Short signals may be profiled as RESEARCH evidence, but A34 portfolio admission remains long-only until borrow/locate/recall economics are accepted.
- **B35 source-only preflight is ACCEPTED; the first authorized DEVELOPMENT read attempt is preserved as an interrupted run and is being repaired without changing the frozen experiment.** The preflight selected exactly `59,768` minute units with source fingerprint `af371478a68d2dca486ffbaa1a339d24e1166257a67ba5f6ededfdda202a04cd`, split-evidence fingerprint `3bcccaddf3937368c675fb27a441f9bbac8cbf34816c26e37a4a3e84d03554f6`, and active B35 fingerprint `145cc8983439b6062fd5e60303539d1611cdf7db0119ed68a7da1e68ffd8eda6`. On `main` `1d00fb9d362f46c226972ae4d92c45c2fd395413`, explicit DEVELOPMENT authorization `562d7104d56151e6203a1bf85457f9d1a90bbf19e60cd4d9359a3f96bc0a7be5` and registration `b35.dev.20160104_20260430.eb3b7ff9f417.registration` were created before the run stopped on `B35 canonical unit contains incorrect session labels`. Review traced this to a validator/writer mismatch: the accepted V2 writer intentionally persists provider bars outside the extended-session clock as `session_segment='closed'`, while the B35 validator incorrectly rejected that fourth canonical label. The repair validates `closed` against the exact writer clock, still rejects mislabeled rows, and materializes only premarket/regular rows into strategy memory. It also adds a hardware-aware research execution profile; on a 12-logical-CPU/24-GiB host the default is eight DuckDB threads while reserving host headroom, and validation scans are collapsed to reduce repeated Parquet I/O. The same immutable authorization/trial identity is reused on restart; no alternate scope or scientific rule is introduced. Consumed-master and future-blind rows remain `0`; provider/broker/PAPER/LIVE authority remains absent; strategy/selector promotion remains `false`.
