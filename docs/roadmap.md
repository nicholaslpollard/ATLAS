...
7. **CURRENT Track-B gate — accept the execution-only B35 observable parallel replay package, then resume the same canonical DEVELOPMENT replay.** Source-only preflight and the session-label semantics repair are accepted. The authorized canonical replay was intentionally interrupted after a measurement snapshot reached **69 / 482 deterministic groups** and **8,556 / 59,768 source units (14.32%)**; the restart portion averaged about **660.6 source units/hour**, implying roughly **77.5 hours** still remaining under the serial coordinator. PR #72 therefore changes execution only: independent frozen symbol-batch groups may run concurrently, while each worker uses the same accepted inner replay mechanics and unique group output/receipt. The parent revalidates every output SHA/self-hash receipt and restores frozen aggregation order. The dedicated 12-logical-CPU/~24-GiB workstation hardware-auto profile targets **5 workers x 2 DuckDB threads = 10 aggregate worker threads**, reserving two logical CPUs and RAM headroom. Existing valid receipts remain reusable. Before merge/resume, the workstation must have no active canonical replay lock, its system clock should be corrected for meaningful operational timestamps, and `python scripts/verify_b35_parallel_equivalence.py --authorize-development-outcomes --groups 5` must pass against completed canonical evidence through the actual parallel path with byte-identical JSONL SHA-256 and equivalent scientific receipt fields. The hard performance objective is <24 hours for a full canonical replay where achievable without weakening safeguards; 8-12 hours is a stretch objective, not authority to reduce quality. Workstation probes preserve exact scientific output at **5/5, 8/8, and 10/10** sampled groups and measure respectively **1,744.4**, **2,560.9**, and **2,676.8 source units/hour** (34.3, 23.3, and 22.3 projected full-replay hours). Because 10 x 1 adds little over 8 x 1, a single-Parquet-scan execution experiment was tested next. It preserved exact scientific equivalence at **10/10** sampled groups but regressed to **820.8 source units/hour** (**72.8 projected full-replay hours / 60.4 remaining**) versus the accepted 10 x 1 baseline **2,676.8 source units/hour** (**22.3 full / 18.5 remaining**). That experiment is **REJECTED** and the source loader is reverted to the proven dual-DuckDB-scan path. The best accepted shape remains 10 x 1. Further execution-only work targets Python/object/strategy processing and narrower per-worker connection/calendar/cache/data-layout reuse; canonical SHA verification, all fail-closed validators, frozen strategy semantics, receipt authority, and exact-equivalence acceptance remain unchanged. The next staged execution-only probe therefore reuses one in-memory DuckDB connection per persistent worker and caches immutable month calendar frames, eliminating repeated process-local setup work without caching source bytes, checkpoint bindings, canonical hashes, or validation results. After exact-head Windows/Ubuntu acceptance and a successful workstation equivalence/performance probe, merge PR #72 and rerun the same canonical `python scripts/run_b35_development_replay.py --authorize-development-outcomes`; it must reuse the immutable authorization `562d7104d56151e6203a1bf85457f9d1a90bbf19e60cd4d9359a3f96bc0a7be5`, trial `b35.dev.20160104_20260430.eb3b7ff9f417.registration`, frozen `2016-01-04..2026-04-30` source, and every still-valid completed receipt. Consumed-master and future-blind rows remain forbidden; provider/broker/PAPER/LIVE and strategy/selector-promotion authority remain zero/false. After successful B35 completion, proceed to strategy x condition evidence and the preregistered selector.
