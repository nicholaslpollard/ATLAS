...
BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_STATUS = "ACCEPTED_NEGATIVE_DEVELOPMENT"
...
BENEFICIAL_OWNERSHIP_EXPECTED_STAGE_COUNTS = {"DEVELOPMENT": 2763, "PROTECTED": 889}
BENEFICIAL_OWNERSHIP_EXPECTED_CANDIDATE_COUNTS = {
    "initial_13d_10_plus_long": 938,
    "initial_13d_5_to_10_long": 742,
    "initial_13g_10_plus_long": 272,
    "initial_13g_5_to_10_long": 1700,
}
BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_START = "2021-08-16"
BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_END = "2023-12-26"
BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_START = "2024-03-28"
BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_END = "2024-12-31"
BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_SESSION_COUNT = 850
BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_SESSION_COUNT = 595
BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_SESSION_COUNT = 192
BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_SESSION_COUNT = 63
BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_FIRST = "2023-12-27"
BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_LAST = "2024-03-27"


class BeneficialOwnershipCloseoutProbeError(RuntimeError):
    pass


def _read_json(path: Path, label: str) -> dict[str, Any]:
    if not path.is_file():
        raise BeneficialOwnershipCloseoutProbeError(f"missing {label}: {path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise BeneficialOwnershipCloseoutProbeError(f"invalid {label}: {path}") from exc
    if not isinstance(value, dict):
        raise BeneficialOwnershipCloseoutProbeError(f"{label} must be a JSON object")
    return value
...
def beneficial_ownership_closeout_disposition(
    *,
    ...
    if protected_return_rows_read != 0 or protected_holdout_consumed:
        raise BeneficialOwnershipCloseoutProbeError(
...
def collect_beneficial_ownership_closeout_evidence(
    settings: AtlasSettings,
) -> dict[str, Any]:
    """Reconcile persisted target-machine artifacts only; perform no provider/outcome reads."""
    ...
    checks: dict[str, bool] = {
        ...
        "development_status_exact": report.get("status") == BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_STATUS,
        "development_report_pass": report.get("pass") is True,
        "predictor_rows_exact": int(predictor.get("predictor_rows", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_PREDICTOR_ROWS,
        "stage_counts_exact": predictor.get("stage_counts") == BENEFICIAL_OWNERSHIP_EXPECTED_STAGE_COUNTS,
        "candidate_counts_exact": predictor.get("candidate_counts") == BENEFICIAL_OWNERSHIP_EXPECTED_CANDIDATE_COUNTS,
        "provider_source_reads_exact": int(predictor.get("provider_source_reads", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_PROVIDER_SOURCE_READS,
        ...
        "development_predictor_rows_exact": int(diagnostics.get("development_predictor_rows_opened", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_PREDICTOR_ROWS,
        "missing_stock_paths_exact": int(diagnostics.get("exact_stock_path_missing_rows", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_MISSING_STOCK_PATH_ROWS,
        "split_censored_rows_exact": int(diagnostics.get("split_crossing_censored_rows", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_SPLIT_CENSORED_ROWS,
        "usable_outcome_rows_exact": int(diagnostics.get("usable_development_rows", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_OUTCOME_ROWS,
        "target_outcome_rows_exact": int(report.get("target_outcome_rows_read", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_OUTCOME_ROWS,
        "protected_predictor_rows_exact": int(report.get("protected_predictor_rows_read_for_source_precheck", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_PROTECTED_PREDICTOR_ROWS,
        ...
        "selection_start_exact": boundaries.get("selection_start") == BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_START,
        "selection_end_exact": boundaries.get("selection_end") == BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_END,
        "internal_start_exact": boundaries.get("internal_start") == BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_START,
        "internal_end_exact": boundaries.get("internal_end") == BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_END,
        "development_session_count_exact": int(boundaries.get("development_session_count", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_SESSION_COUNT,
        "selection_session_count_exact": int(boundaries.get("selection_session_count", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_SELECTION_SESSION_COUNT,
        "internal_session_count_exact": int(boundaries.get("internal_session_count", -1)) == BENEFICIAL_OWNERSHIP_EXPECTED_INTERNAL_SESSION_COUNT,
        "purge_session_count_exact": len(purge) == BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_SESSION_COUNT,
        "purge_first_exact": bool(purge) and purge[0] == BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_FIRST,
        "purge_last_exact": bool(purge) and purge[-1] == BENEFICIAL_OWNERSHIP_EXPECTED_PURGE_LAST,
    ...
    if not all(checks.values()):
        ...
        raise BeneficialOwnershipCloseoutProbeError(
            "beneficial-ownership closeout evidence probe failed: " + ", ".join(failed)
        )

    evidence = {
        "probe_contract": BENEFICIAL_OWNERSHIP_CLOSEOUT_PROBE_CONTRACT,
        ...
        **hashes,
        ...
        "predictor_rows": BENEFICIAL_OWNERSHIP_EXPECTED_PREDICTOR_ROWS,
        "development_predictor_rows": BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_PREDICTOR_ROWS,
        "development_outcome_rows": BENEFICIAL_OWNERSHIP_EXPECTED_DEVELOPMENT_OUTCOME_ROWS,
        "protected_predictor_rows": BENEFICIAL_OWNERSHIP_EXPECTED_PROTECTED_PREDICTOR_ROWS,
        "selection_passers": [],
        "selection_winners": [],
        "internal_finalists": [],
    ...
    return {
        "contract_version": BENEFICIAL_OWNERSHIP_CLOSEOUT_PROBE_CONTRACT,
        "pass": True,
        "ready_to_pin": True,
        "disposition": disposition,
