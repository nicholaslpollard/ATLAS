...
from packages.backtesting.alpha_gate_beneficial_ownership_predictor import (
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_ROOT_RELATIVE,
...
from packages.backtesting.alpha_gate_beneficial_ownership_scientific_policy import (
    BENEFICIAL_OWNERSHIP_BOOTSTRAP_BLOCK_SESSIONS,
    BENEFICIAL_OWNERSHIP_BOOTSTRAP_REPLICATES,
    BENEFICIAL_OWNERSHIP_BOOTSTRAP_SEED,
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_LAST_SIGNAL,
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_SELECTION_FRACTION,
    BENEFICIAL_OWNERSHIP_HYPOTHESES,
    BENEFICIAL_OWNERSHIP_INTERNAL_CONFIDENCE,
    BENEFICIAL_OWNERSHIP_INTERNAL_MIN_EVENT_ROWS,
    BENEFICIAL_OWNERSHIP_INTERNAL_MIN_POSITIVE_FOLDS,
    BENEFICIAL_OWNERSHIP_INTERNAL_MIN_SIGNAL_SESSIONS,
    BENEFICIAL_OWNERSHIP_INTERNAL_MIN_UNIQUE_INSTRUMENTS,
    BENEFICIAL_OWNERSHIP_INTERNAL_PURGE_SESSIONS,
    BENEFICIAL_OWNERSHIP_INTERNAL_VALIDATION_FOLDS,
    BENEFICIAL_OWNERSHIP_MAX_SINGLE_INSTRUMENT_ROW_FRACTION,
    BENEFICIAL_OWNERSHIP_MAX_SINGLE_SESSION_ROW_FRACTION,
    BENEFICIAL_OWNERSHIP_MIN_POSITIVE_YEAR_FRACTION,
    BENEFICIAL_OWNERSHIP_MIN_YEAR_SIGNAL_SESSIONS,
    BENEFICIAL_OWNERSHIP_MULTIPLE_TESTING_ALPHA,
    BENEFICIAL_OWNERSHIP_MULTIPLE_TESTING_METHOD,
    BENEFICIAL_OWNERSHIP_OUTER_EMBARGO_END,
    BENEFICIAL_OWNERSHIP_PERFORMANCE_SIGNAL_START,
    BENEFICIAL_OWNERSHIP_PRIMARY_COST_BPS,
    BENEFICIAL_OWNERSHIP_PRIMARY_HORIZON_SESSIONS,
    ...
    BENEFICIAL_OWNERSHIP_SELECTION_CONFIDENCE,
    BENEFICIAL_OWNERSHIP_SELECTION_FOLDS,
    BENEFICIAL_OWNERSHIP_SELECTION_MIN_EVENT_ROWS,
    BENEFICIAL_OWNERSHIP_SELECTION_MIN_POSITIVE_FOLDS,
    BENEFICIAL_OWNERSHIP_SELECTION_MIN_SIGNAL_SESSIONS,
    BENEFICIAL_OWNERSHIP_SELECTION_MIN_UNIQUE_INSTRUMENTS,
    BENEFICIAL_OWNERSHIP_SELECTION_WINNER_RULE,
    BENEFICIAL_OWNERSHIP_STRESS_COST_BPS,
    beneficial_ownership_scientific_fingerprint,
)
...
BENEFICIAL_OWNERSHIP_DEVELOPMENT_CONTRACT = (
    "alpha-gate-beneficial-ownership-development-v1-63-session-spy-relative-protected-blind"
)
BENEFICIAL_OWNERSHIP_OUTCOME_CONTRACT = (
    "alpha-gate-beneficial-ownership-outcome-v1-exact-open-t63-close-spy-relative-split-censored"
)
BENEFICIAL_OWNERSHIP_FINALIST_CONTRACT = (
    "alpha-gate-beneficial-ownership-finalists-v1-selection-internal-protected-source-precheck-returns-unread"
)
...
BENEFICIAL_OWNERSHIP_DEVELOPMENT_OUTCOMES_RELATIVE = (
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_ROOT_RELATIVE / "development_outcomes.jsonl"
)
BENEFICIAL_OWNERSHIP_FINALISTS_RELATIVE = (
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_ROOT_RELATIVE / "finalists.json"
)
BENEFICIAL_OWNERSHIP_DEVELOPMENT_REPORT_RELATIVE = (
    BENEFICIAL_OWNERSHIP_DEVELOPMENT_ROOT_RELATIVE / "development_study.json"
)
...
class BeneficialOwnershipDevelopmentError(RuntimeError):
    pass
...
def development_implementation_fingerprint() -> str:
    payload = {
        ...
        "predictor_contract": BENEFICIAL_OWNERSHIP_PREDICTOR_CONTRACT,
        "development_contract": BENEFICIAL_OWNERSHIP_DEVELOPMENT_CONTRACT,
        "outcome_contract": BENEFICIAL_OWNERSHIP_OUTCOME_CONTRACT,
        "finalist_contract": BENEFICIAL_OWNERSHIP_FINALIST_CONTRACT,
        "split_policy": "accepted_phase26_split_evidence_censor_decision_open_to_t63_close",
        "selection_rule": "selection_only_global_holm_then_single_long_winner_internal_confirm",
...
def _load_jsonl(path: Path) -> tuple[dict[str, Any], ...]:
    if not path.is_file():
        raise BeneficialOwnershipDevelopmentError(f"missing JSONL artifact: {path}")
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        try:
            ...
            raise BeneficialOwnershipDevelopmentError(
...
def chronological_boundaries(sessions: Iterable[date]) -> DevelopmentBoundaries:
    ...
    if len(ordered) < 200:
        raise BeneficialOwnershipDevelopmentError("too few development sessions")
    selection_count = int(math.floor(len(ordered) * BENEFICIAL_OWNERSHIP_DEVELOPMENT_SELECTION_FRACTION))
    internal_offset = selection_count + BENEFICIAL_OWNERSHIP_INTERNAL_PURGE_SESSIONS
    if selection_count <= 0 or internal_offset >= len(ordered):
        raise BeneficialOwnershipDevelopmentError("invalid selection/internal chronology")
    selection = ordered[:selection_count]
    purge = ordered[selection_count:internal_offset]
    internal = ordered[internal_offset:]
    if len(purge) != BENEFICIAL_OWNERSHIP_INTERNAL_PURGE_SESSIONS or not internal:
        raise BeneficialOwnershipDevelopmentError("internal purge partition is incomplete")
...
def holm_bonferroni(
    p_values: Mapping[str, float], *, alpha: float = BENEFICIAL_OWNERSHIP_MULTIPLE_TESTING_ALPHA
...
def _fold_mapping(sessions: tuple[date, ...], folds: int) -> dict[date, int]:
    if len(sessions) < folds:
        raise BeneficialOwnershipDevelopmentError("too few sessions for fold attribution")
...
def _derived_seed(label: str) -> int:
    return BENEFICIAL_OWNERSHIP_BOOTSTRAP_SEED + int(
...
def _bootstrap(values: np.ndarray, *, confidence: float, label: str) -> tuple[float, float]:
    if values.ndim != 1 or len(values) == 0:
        raise BeneficialOwnershipDevelopmentError("bootstrap requires a nonempty session vector")
    n = len(values)
    block = min(BENEFICIAL_OWNERSHIP_BOOTSTRAP_BLOCK_SESSIONS, n)
    block_count = int(math.ceil(n / block))
    rng = np.random.default_rng(_derived_seed(label))
    starts = rng.integers(0, n, size=(BENEFICIAL_OWNERSHIP_BOOTSTRAP_REPLICATES, block_count))
    offsets = np.arange(block, dtype=np.int64)
    indices = ((starts[:, :, None] + offsets) % n).reshape(
        BENEFICIAL_OWNERSHIP_BOOTSTRAP_REPLICATES, -1
    )[:, :n]
...
def _empty_metrics(folds: int) -> TrancheMetrics:
...
def tranche_metrics(
    ...
    *,
    ...
    if set(data["direction"].astype(str)) != {"LONG"}:
        raise BeneficialOwnershipDevelopmentError("candidate direction drifted from frozen LONG")
    for field in ("primary_gross_return", "unhedged_gross_return"):
        data[field] = pd.to_numeric(data[field], errors="coerce")
    finite = np.isfinite(data["primary_gross_return"].to_numpy(dtype=float)) & np.isfinite(
        data["unhedged_gross_return"].to_numpy(dtype=float)
    )
    data = data.loc[finite].copy()
    if data.empty:
        return _empty_metrics(fold_count)

    primary_cost = BENEFICIAL_OWNERSHIP_PRIMARY_COST_BPS / 10_000.0
    stress_cost = BENEFICIAL_OWNERSHIP_STRESS_COST_BPS / 10_000.0
    session = (
    ...
    primary = session["primary_gross_return"].to_numpy(dtype=float) - primary_cost
    unhedged = session["unhedged_gross_return"].to_numpy(dtype=float) - primary_cost
    stress = session["primary_gross_return"].to_numpy(dtype=float) - stress_cost
    ...
    if session["fold"].isna().any():
        raise BeneficialOwnershipDevelopmentError("signal session is missing fold attribution")
    fold_values: list[float | None] = []
    for fold in range(fold_count):
        group = session.loc[session["fold"] == fold, "primary_gross_return"]
        fold_values.append(
    ...
    year_means = {
        ...
        if len(values) >= BENEFICIAL_OWNERSHIP_MIN_YEAR_SIGNAL_SESSIONS
    }
    positive_year_fraction = (
        None
        if not year_means
        else float(sum(value > 0 for value in year_means.values()) / len(year_means))
    )
    raw_rows = int(len(data))
    instrument_counts = data.groupby("instrument_id", sort=True, observed=True).size()
    session_std = float(np.std(primary, ddof=1)) if len(primary) > 1 else 0.0
    return TrancheMetrics(
        raw_rows=raw_rows,
        ...
        session_sharpe=(
            None if session_std <= 0 else float(np.mean(primary) / session_std)
        ),
    )


def with_deflated_diagnostic(
    metrics: Mapping[str, TrancheMetrics],
) -> dict[str, TrancheMetrics]:
    sharpes = np.asarray(
    ...
    result: dict[str, TrancheMetrics] = {}
    ...
    *,
    ...
    return {
        ...
        "year_robustness": bool(
            ...
            and metrics.positive_year_fraction >= BENEFICIAL_OWNERSHIP_MIN_POSITIVE_YEAR_FRACTION
        ),
...
def selection_checks(metrics: TrancheMetrics) -> dict[str, bool]:
    return _stage_checks(
        metrics,
        min_event_rows=BENEFICIAL_OWNERSHIP_SELECTION_MIN_EVENT_ROWS,
        min_signal_sessions=BENEFICIAL_OWNERSHIP_SELECTION_MIN_SIGNAL_SESSIONS,
        min_unique_instruments=BENEFICIAL_OWNERSHIP_SELECTION_MIN_UNIQUE_INSTRUMENTS,
        min_positive_folds=BENEFICIAL_OWNERSHIP_SELECTION_MIN_POSITIVE_FOLDS,
    )
...
def internal_checks(metrics: TrancheMetrics) -> dict[str, bool]:
    return _stage_checks(
        metrics,
        min_event_rows=BENEFICIAL_OWNERSHIP_INTERNAL_MIN_EVENT_ROWS,
        min_signal_sessions=BENEFICIAL_OWNERSHIP_INTERNAL_MIN_SIGNAL_SESSIONS,
        min_unique_instruments=BENEFICIAL_OWNERSHIP_INTERNAL_MIN_UNIQUE_INSTRUMENTS,
        min_positive_folds=BENEFICIAL_OWNERSHIP_INTERNAL_MIN_POSITIVE_FOLDS,
    )


def protected_source_precheck(rows: pd.DataFrame, candidate_id: str) -> dict[str, object]:
    subset = rows.loc[rows["candidate_id"].astype(str).eq(candidate_id)].copy()
    if subset.empty:
        raw_rows = signal_sessions = unique_instruments = 0
        max_session_fraction = max_instrument_fraction = None
    else:
        subset["decision_session"] = pd.to_datetime(subset["decision_session"]).dt.date
        ...
        max_session_fraction = float(
            subset.groupby("decision_session", observed=True).size().max() / raw_rows
        )
        max_instrument_fraction = float(
            subset.groupby("instrument_id", observed=True).size().max() / raw_rows
        )
    gates = {
        "min_event_rows": raw_rows >= BENEFICIAL_OWNERSHIP_PROTECTED_MIN_EVENT_ROWS,
        "min_signal_sessions": signal_sessions >= BENEFICIAL_OWNERSHIP_PROTECTED_MIN_SIGNAL_SESSIONS,
        "min_unique_instruments": unique_instruments
        >= BENEFICIAL_OWNERSHIP_PROTECTED_MIN_UNIQUE_INSTRUMENTS,
    }
    return {
        ...
        "max_single_instrument_row_fraction_diagnostic": max_instrument_fraction,
        "gates": gates,
...
def regulatory_era_diagnostic(rows: pd.DataFrame) -> dict[str, dict[str, float | int | None]]:
    boundary = date(2024, 2, 5)
    output: dict[str, dict[str, float | int | None]] = {}
    cost = BENEFICIAL_OWNERSHIP_PRIMARY_COST_BPS / 10_000.0
    for label, mask in (
        ("pre_2024_02_05", rows["decision_session"] < boundary),
        ("post_2024_02_05", rows["decision_session"] >= boundary),
    ):
        subset = rows.loc[mask]
        output[label] = {
            "rows": int(len(subset)),
            "signal_sessions": int(subset["decision_session"].nunique()) if not subset.empty else 0,
            "primary_mean_after_cost": (
                None
                if subset.empty
                else float(pd.to_numeric(subset["primary_gross_return"]).mean() - cost)
            ),
        }
    return output


class BeneficialOwnershipDevelopmentStudy:
    """Open development returns only; protected returns remain unread."""

    def __init__(self, settings: AtlasSettings, *, progress_callback: Any | None = None) -> None:
        ...
        self.predictor_rows_path = self.derived_root / BENEFICIAL_OWNERSHIP_PREDICTOR_ROWS_RELATIVE
        self.predictor_report_path = self.derived_root / BENEFICIAL_OWNERSHIP_PREDICTOR_REPORT_RELATIVE
    ...
    def _read_json(self, path: Path, label: str) -> dict[str, Any]:
        if not path.is_file():
            raise BeneficialOwnershipDevelopmentError(f"missing {label}: {path}")
        value = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(value, dict):
            raise BeneficialOwnershipDevelopmentError(f"{label} must be a JSON object")
        return value

    def _load_predictors(self) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
        report = self._read_json(self.predictor_report_path, "beneficial-ownership predictor report")
        if report.get("contract_version") != BENEFICIAL_OWNERSHIP_PREDICTOR_CONTRACT:
            raise BeneficialOwnershipDevelopmentError("predictor contract mismatch")
        if report.get("scientific_fingerprint") != BENEFICIAL_OWNERSHIP_SCIENTIFIC_FINGERPRINT:
            raise BeneficialOwnershipDevelopmentError("predictor scientific fingerprint mismatch")
        if int(report.get("target_outcome_rows_read", -1)) != 0:
            raise BeneficialOwnershipDevelopmentError("predictor artifact was not outcome-blind")
        if int(report.get("protected_return_rows_read", -1)) != 0:
            raise BeneficialOwnershipDevelopmentError("predictor artifact consumed protected returns")
        rows = _load_jsonl(self.predictor_rows_path)
        if sha256_file(self.predictor_rows_path) != str(report.get("predictor_rows_sha256") or ""):
            raise BeneficialOwnershipDevelopmentError("predictor rows SHA differs from source-only report")
        if len(rows) != int(report.get("predictor_rows", -1)):
            raise BeneficialOwnershipDevelopmentError("predictor row count differs from report")
        valid_candidates = {spec.candidate_id for spec in BENEFICIAL_OWNERSHIP_HYPOTHESES}
        for row in rows:
            if row.get("scientific_fingerprint") != BENEFICIAL_OWNERSHIP_SCIENTIFIC_FINGERPRINT:
                raise BeneficialOwnershipDevelopmentError("predictor row fingerprint drifted")
            if str(row.get("candidate_id") or "") not in valid_candidates:
                raise BeneficialOwnershipDevelopmentError("predictor contains non-frozen candidate")
            forbidden = _FORBIDDEN_PREDICTOR_FIELDS.intersection(row)
            if forbidden:
                raise BeneficialOwnershipDevelopmentError(
                    "predictor contains forbidden outcome fields: " + ", ".join(sorted(forbidden))
                )
        development = pd.DataFrame.from_records(
            [dict(row) for row in rows if str(row.get("stage") or "") == "DEVELOPMENT"]
        )
        protected = pd.DataFrame.from_records(
            [dict(row) for row in rows if str(row.get("stage") or "") == "PROTECTED"]
        )
        if development.empty:
            raise BeneficialOwnershipDevelopmentError("development predictor population is empty")
        development["decision_session"] = pd.to_datetime(development["decision_session"]).dt.date
        if development["decision_session"].max() > date.fromisoformat(
            BENEFICIAL_OWNERSHIP_DEVELOPMENT_LAST_SIGNAL
        ):
            raise BeneficialOwnershipDevelopmentError("development predictor crossed frozen boundary")
        if not protected.empty:
            protected["decision_session"] = pd.to_datetime(protected["decision_session"]).dt.date
            if protected["decision_session"].min() < date.fromisoformat(
                BENEFICIAL_OWNERSHIP_PROTECTED_START
            ):
                raise BeneficialOwnershipDevelopmentError("protected predictor crossed frozen start")
        return development, protected, report

    def _split_evidence(self) -> tuple[pd.DataFrame, str]:
        report = self._read_json(self.phase26.report_path(), "accepted Phase26 observation report")
        if (
            report.get("contract_version") != PHASE26_OBSERVATION_REPORT_CONTRACT_VERSION
            or report.get("pass") is not True
        ):
            raise BeneficialOwnershipDevelopmentError("accepted Phase26 split evidence is unavailable")
        if int(report.get("protected_return_reads", -1)) != 0:
            raise BeneficialOwnershipDevelopmentError("split evidence is attached to consumed protected returns")
        ...
        if not path.is_file() or len(expected_sha) != 64 or sha256_file(path) != expected_sha:
            raise BeneficialOwnershipDevelopmentError("accepted split evidence SHA mismatch")
    ...
    ) -> tuple[DevelopmentBoundaries, pd.DataFrame, tuple[date, ...], tuple[date, ...]]:
        start = date.fromisoformat(BENEFICIAL_OWNERSHIP_PERFORMANCE_SIGNAL_START)
        development_last = date.fromisoformat(BENEFICIAL_OWNERSHIP_DEVELOPMENT_LAST_SIGNAL)
        outer_end = date.fromisoformat(BENEFICIAL_OWNERSHIP_OUTER_EMBARGO_END)
        development_sessions = tuple(self.calendar.sessions_in_range(start, development_last))
        boundary = chronological_boundaries(development_sessions)
        full_grid = tuple(self.calendar.sessions_in_range(start, outer_end))
        positions = {session: index for index, session in enumerate(full_grid)}
        frame = predictors.copy()
        exits: list[date] = []
        for decision in frame["decision_session"]:
            index = positions.get(decision)
            if index is None or index + BENEFICIAL_OWNERSHIP_PRIMARY_HORIZON_SESSIONS >= len(full_grid):
                raise BeneficialOwnershipDevelopmentError(
                    f"cannot resolve frozen 63-session exit for {decision}"
                )
            exits.append(full_grid[index + BENEFICIAL_OWNERSHIP_PRIMARY_HORIZON_SESSIONS])
        frame["exit_session"] = exits
        if frame["exit_session"].max() > outer_end:
            raise BeneficialOwnershipDevelopmentError("development outcome crossed outer embargo end")
        selection_sessions = tuple(
            session
            for session in development_sessions
            if boundary.selection_start <= session <= boundary.selection_end
        )
        internal_sessions = tuple(
            session
            for session in development_sessions
            if boundary.internal_start <= session <= boundary.internal_end
        )
        return boundary, frame, selection_sessions, internal_sessions

    def _development_outcomes(
        self, predictors: pd.DataFrame, splits: pd.DataFrame
    ) -> tuple[pd.DataFrame, dict[str, int]]:
        self._progress(
            f"opening development-only exact entry/exit paths for {len(predictors)} predictor rows"
        )
        columns = [
            "candidate_id",
            "direction",
            ...
            "source_era",
            "subject_cik",
            "reported_percent_of_class",
        ]
        query = predictors[columns].copy().reset_index(drop=True)
        query.insert(0, "row_id", np.arange(len(query), dtype=np.int64))
        bar_glob = self.paths.glob_for_timeframe(Timeframe.DAY_1)
        con = connect_utc(":memory:")
        try:
            con.execute("SET preserve_insertion_order=false")
            con.register("beneficial_predictors", query)
            con.register("beneficial_splits", splits)
            result = con.execute(
                f"""
                WITH needed AS (
                    SELECT ticker AS symbol, CAST(decision_session AS DATE) AS session_date
                    FROM beneficial_predictors
                    UNION SELECT ticker, CAST(exit_session AS DATE) FROM beneficial_predictors
                    UNION SELECT 'SPY', CAST(decision_session AS DATE) FROM beneficial_predictors
                    UNION SELECT 'SPY', CAST(exit_session AS DATE) FROM beneficial_predictors
                ), bars AS (
                ...
                SELECT p.*, se.session_open AS entry_open, sx.session_close AS exit_close,
                       ...
                       EXISTS (
                           SELECT 1 FROM beneficial_splits s
        ...
        if len(result) != len(query):
            raise BeneficialOwnershipDevelopmentError(
                "exact outcome join cardinality drifted; duplicate daily keys suspected"
            )
        ...
        if usable.empty:
            raise BeneficialOwnershipDevelopmentError(
                "development outcomes are empty after path-quality censoring"
            )
        usable["stock_return"] = usable["exit_close"] / usable["entry_open"] - 1.0
        usable["spy_return"] = usable["spy_exit_close"] / usable["spy_entry_open"] - 1.0
        usable["primary_gross_return"] = usable["stock_return"] - usable["spy_return"]
        usable["unhedged_gross_return"] = usable["stock_return"]
        for field in (
            "stock_return",
            "spy_return",
            "primary_gross_return",
            ...
            if not np.isfinite(usable[field].to_numpy(dtype=float)).all():
                raise BeneficialOwnershipDevelopmentError(f"nonfinite development {field}")
        usable.insert(0, "outcome_contract_version", BENEFICIAL_OWNERSHIP_OUTCOME_CONTRACT)
    ...
    def run(self) -> dict[str, Any]:
        if beneficial_ownership_scientific_fingerprint() != BENEFICIAL_OWNERSHIP_SCIENTIFIC_FINGERPRINT:
            raise BeneficialOwnershipDevelopmentError("scientific policy fingerprint drifted")
        if development_implementation_fingerprint() != BENEFICIAL_OWNERSHIP_DEVELOPMENT_IMPLEMENTATION_FINGERPRINT:
            raise BeneficialOwnershipDevelopmentError("development implementation fingerprint drifted")
        if BENEFICIAL_OWNERSHIP_PROTECTED_RETURNS_BEFORE_FINALIST_ALLOWED:
            raise BeneficialOwnershipDevelopmentError("protected-return blindness contract drifted")
        if BENEFICIAL_OWNERSHIP_RUNNER_UP_SUBSTITUTION_ALLOWED:
            raise BeneficialOwnershipDevelopmentError("runner-up substitution contract drifted")
        if BENEFICIAL_OWNERSHIP_MULTIPLE_TESTING_METHOD != "HOLM_BONFERRONI_GLOBAL_4":
            raise BeneficialOwnershipDevelopmentError("multiplicity method drifted")
        if BENEFICIAL_OWNERSHIP_SELECTION_WINNER_RULE != "highest_primary_selection_LCB_then_candidate_id":
            raise BeneficialOwnershipDevelopmentError("selection winner rule drifted")

        development, protected, predictor_report = self._load_predictors()
        boundary, development, selection_sessions, internal_sessions = self._boundaries_and_exits(
            development
        )
        splits, split_sha = self._split_evidence()
        outcomes, outcome_diagnostics = self._development_outcomes(development, splits)

        selection_mapping = _fold_mapping(selection_sessions, BENEFICIAL_OWNERSHIP_SELECTION_FOLDS)
        internal_mapping = _fold_mapping(
            internal_sessions, BENEFICIAL_OWNERSHIP_INTERNAL_VALIDATION_FOLDS
        )
        selection_metrics: dict[str, TrancheMetrics] = {}
        selection_gate_results: dict[str, dict[str, bool]] = {}
        for spec in BENEFICIAL_OWNERSHIP_HYPOTHESES:
            candidate = outcomes.loc[
                outcomes["candidate_id"].astype(str).eq(spec.candidate_id)
            ...
            metrics = tranche_metrics(
                candidate,
                confidence=BENEFICIAL_OWNERSHIP_SELECTION_CONFIDENCE,
                fold_mapping=selection_mapping,
                fold_count=BENEFICIAL_OWNERSHIP_SELECTION_FOLDS,
                label=f"selection:{spec.candidate_id}",
            )
            selection_metrics[spec.candidate_id] = metrics
            selection_gate_results[spec.candidate_id] = selection_checks(metrics)
        selection_metrics = with_deflated_diagnostic(selection_metrics)
        ...
        selection_passers = [
            spec.candidate_id
            for spec in BENEFICIAL_OWNERSHIP_HYPOTHESES
            if all(selection_gate_results[spec.candidate_id].values())
            and bool(holm[spec.candidate_id]["rejected_null"])
        ]
        ranked_passers = sorted(
            selection_passers,
            key=lambda candidate_id: (
                -float(selection_metrics[candidate_id].primary_lcb or float("-inf")),
                candidate_id,
            ),
        )
        winners = ranked_passers[:1]

        internal_metrics: dict[str, TrancheMetrics] = {}
        internal_gate_results: dict[str, dict[str, bool]] = {}
        ...
        for candidate_id in winners:
            ...
            metrics = tranche_metrics(
                candidate,
                confidence=BENEFICIAL_OWNERSHIP_INTERNAL_CONFIDENCE,
                fold_mapping=internal_mapping,
                fold_count=BENEFICIAL_OWNERSHIP_INTERNAL_VALIDATION_FOLDS,
        ...
        protected_prechecks = {
            candidate_id: protected_source_precheck(protected, candidate_id)
        ...
        era_diagnostics = {
            spec.candidate_id: regulatory_era_diagnostic(
                outcomes.loc[
                    outcomes["candidate_id"].astype(str).eq(spec.candidate_id)
                ].copy()
            )
        ...
        outcome_text = "".join(_canonical_json(row) + "\n" for row in outcome_records)
        outcome_path = self.derived_root / BENEFICIAL_OWNERSHIP_DEVELOPMENT_OUTCOMES_RELATIVE
        outcome_path.parent.mkdir(parents=True, exist_ok=True)
        atomic_write_text(outcome_path, outcome_text)

        finalist_artifact = {
            "contract_version": BENEFICIAL_OWNERSHIP_FINALIST_CONTRACT,
            "scientific_fingerprint": BENEFICIAL_OWNERSHIP_SCIENTIFIC_FINGERPRINT,
            ...
            "protected_return_eligible_finalists": protected_return_eligible,
            "runner_up_substitution_allowed": False,
            "protected_return_rows_read": 0,
            "protected_holdout_consumed": False,
        }
        finalist_path = self.derived_root / BENEFICIAL_OWNERSHIP_FINALISTS_RELATIVE
        atomic_write_text(finalist_path, json.dumps(finalist_artifact, indent=2, sort_keys=True) + "\n")
        ...
        report = {
            "contract_version": BENEFICIAL_OWNERSHIP_DEVELOPMENT_CONTRACT,
            "scientific_fingerprint": BENEFICIAL_OWNERSHIP_SCIENTIFIC_FINGERPRINT,
            "development_implementation_fingerprint": BENEFICIAL_OWNERSHIP_DEVELOPMENT_IMPLEMENTATION_FINGERPRINT,
            "status": status,
            "pass": True,
            "predictor_contract": BENEFICIAL_OWNERSHIP_PREDICTOR_CONTRACT,
            "predictor_report_sha256": sha256_file(self.predictor_report_path),
            "predictor_rows_sha256": sha256_file(self.predictor_rows_path),
            "predictor_source_reads_performed": int(
                predictor_report.get("provider_source_reads", 0)
            ),
            ...
            "internal_finalists": internal_finalists,
            "regulatory_era_diagnostic_contract": BENEFICIAL_OWNERSHIP_REGULATORY_ERA_DIAGNOSTIC,
            "regulatory_era_diagnostics": era_diagnostics,
            "protected_source_prechecks": protected_prechecks,
            "protected_return_eligible_finalists": protected_return_eligible,
            "development_outcomes_path": str(outcome_path),
            "development_outcomes_sha256": _sha256_text(outcome_text),
            "finalists_path": str(finalist_path),
            "finalists_sha256": sha256_file(finalist_path),
            "target_outcome_rows_read": int(len(outcomes)),
            "protected_predictor_rows_read_for_source_precheck": int(len(protected)),
            "protected_return_rows_read": 0,
            "protected_holdout_consumed": False,
            ...
            "next_scientific_action": (
                "If the single finalist passes the frozen protected source-only precheck, open protected returns once for only that finalist; otherwise close this mechanism without support."
            ),
        }
        report_path = self.derived_root / BENEFICIAL_OWNERSHIP_DEVELOPMENT_REPORT_RELATIVE
