...
from packages.backtesting.alpha_gate_xbrl_scientific_policy import (
    XBRL_BOOTSTRAP_BLOCK_SESSIONS,
    XBRL_BOOTSTRAP_REPLICATES,
    XBRL_BOOTSTRAP_SEED,
    XBRL_DEVELOPMENT_LAST_SIGNAL,
    XBRL_DEVELOPMENT_SELECTION_FRACTION,
    XBRL_HYPOTHESES,
    XBRL_INTERNAL_CONFIDENCE,
    XBRL_INTERNAL_MIN_EVENT_ROWS,
    XBRL_INTERNAL_MIN_POSITIVE_FOLDS,
    XBRL_INTERNAL_MIN_SIGNAL_SESSIONS,
    XBRL_INTERNAL_MIN_UNIQUE_INSTRUMENTS,
    XBRL_INTERNAL_PURGE_SESSIONS,
    XBRL_INTERNAL_VALIDATION_FOLDS,
    XBRL_MAX_SINGLE_INSTRUMENT_ROW_FRACTION,
    XBRL_MAX_SINGLE_SESSION_ROW_FRACTION,
    XBRL_MIN_POSITIVE_YEAR_FRACTION,
    XBRL_MIN_YEAR_SIGNAL_SESSIONS,
    XBRL_MULTIPLE_TESTING_ALPHA,
    XBRL_MULTIPLE_TESTING_METHOD,
    XBRL_OUTER_EMBARGO_END,
    XBRL_PERFORMANCE_SIGNAL_START,
    XBRL_PRIMARY_COST_BPS,
    XBRL_PRIMARY_HORIZON_SESSIONS,
    XBRL_PROTECTED_MIN_EVENT_ROWS,
    XBRL_PROTECTED_MIN_SIGNAL_SESSIONS,
    XBRL_PROTECTED_MIN_UNIQUE_INSTRUMENTS,
    ...
    XBRL_SELECTION_CONFIDENCE,
    XBRL_SELECTION_FOLDS,
    XBRL_SELECTION_MIN_EVENT_ROWS,
    XBRL_SELECTION_MIN_POSITIVE_FOLDS,
    XBRL_SELECTION_MIN_SIGNAL_SESSIONS,
    XBRL_SELECTION_MIN_UNIQUE_INSTRUMENTS,
...
XBRL_DEVELOPMENT_CONTRACT = "alpha-gate-xbrl-development-v1-63-session-spy-relative-protected-blind"
XBRL_OUTCOME_CONTRACT = "alpha-gate-xbrl-outcome-v1-exact-open-t63-close-spy-relative-split-censored"
XBRL_FINALIST_CONTRACT = "alpha-gate-xbrl-finalists-v1-selection-internal-protected-source-precheck-returns-unread"
XBRL_DEVELOPMENT_IMPLEMENTATION_FINGERPRINT = "3b5a02113ceab0065ea9a03020cc5266222e67ba39abe36311a6959e7e2d488f"
XBRL_DEVELOPMENT_ROOT_RELATIVE = Path("strategy_evaluation/pre_phase33/xbrl_development_v1")
XBRL_DEVELOPMENT_OUTCOMES_RELATIVE = XBRL_DEVELOPMENT_ROOT_RELATIVE / "development_outcomes.jsonl"
XBRL_FINALISTS_RELATIVE = XBRL_DEVELOPMENT_ROOT_RELATIVE / "finalists.json"
XBRL_DEVELOPMENT_REPORT_RELATIVE = XBRL_DEVELOPMENT_ROOT_RELATIVE / "development_study.json"
...
class XBRLDevelopmentError(RuntimeError):
    pass


@dataclass(frozen=True, slots=True)
class XBRLDevelopmentBoundaries:
...
def development_implementation_fingerprint() -> str:
    payload = {
        ...
        "predictor_contract": XBRL_PREDICTOR_CONTRACT,
        "development_contract": XBRL_DEVELOPMENT_CONTRACT,
        "outcome_contract": XBRL_OUTCOME_CONTRACT,
        "finalist_contract": XBRL_FINALIST_CONTRACT,
        "split_policy": "accepted_phase26_split_evidence_censor_decision_open_to_t63_close",
        "selection_rule": "selection_only_holm_then_direction_winner_internal_confirm",
...
def _load_jsonl(path: Path) -> tuple[dict[str, Any], ...]:
    if not path.is_file():
        raise XBRLDevelopmentError(f"missing JSONL artifact: {path}")
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        try:
            ...
            raise XBRLDevelopmentError(f"invalid JSONL row: {path}:{line_number}") from exc
        if not isinstance(value, dict):
            raise XBRLDevelopmentError(f"JSONL row is not an object: {path}:{line_number}")
        rows.append(value)
    return tuple(rows)


def chronological_boundaries(sessions: Iterable[date]) -> XBRLDevelopmentBoundaries:
    ordered = tuple(sorted(set(sessions)))
    if len(ordered) < 200:
        raise XBRLDevelopmentError("too few XBRL development sessions")
    selection_count = int(math.floor(len(ordered) * XBRL_DEVELOPMENT_SELECTION_FRACTION))
    internal_offset = selection_count + XBRL_INTERNAL_PURGE_SESSIONS
    if selection_count <= 0 or internal_offset >= len(ordered):
        raise XBRLDevelopmentError("invalid XBRL selection/internal chronology")
    selection = ordered[:selection_count]
    purge = ordered[selection_count:internal_offset]
    internal = ordered[internal_offset:]
    if len(purge) != XBRL_INTERNAL_PURGE_SESSIONS or not internal:
        raise XBRLDevelopmentError("XBRL internal purge partition is incomplete")
    return XBRLDevelopmentBoundaries(
...
def holm_bonferroni(
    p_values: Mapping[str, float], *, alpha: float = XBRL_MULTIPLE_TESTING_ALPHA
...
def _fold_mapping(sessions: tuple[date, ...], folds: int) -> dict[date, int]:
    if len(sessions) < folds:
        raise XBRLDevelopmentError("too few sessions for XBRL fold attribution")
...
def _derived_seed(label: str) -> int:
    return XBRL_BOOTSTRAP_SEED + int(hashlib.sha256(label.encode("utf-8")).hexdigest()[:8], 16)


def _bootstrap(values: np.ndarray, *, confidence: float, label: str) -> tuple[float, float]:
    if values.ndim != 1 or len(values) == 0:
        raise XBRLDevelopmentError("bootstrap requires a nonempty session vector")
    n = len(values)
    block = min(XBRL_BOOTSTRAP_BLOCK_SESSIONS, n)
    block_count = int(math.ceil(n / block))
    rng = np.random.default_rng(_derived_seed(label))
    starts = rng.integers(0, n, size=(XBRL_BOOTSTRAP_REPLICATES, block_count))
    offsets = np.arange(block, dtype=np.int64)
    indices = ((starts[:, :, None] + offsets) % n).reshape(
        XBRL_BOOTSTRAP_REPLICATES, -1
    )[:, :n]
...
def _empty_metrics(folds: int) -> XBRLTrancheMetrics:
    return XBRLTrancheMetrics(
        raw_rows=0,
        signal_sessions=0,
...
def tranche_metrics(
    ...
    *,
    ...
    if direction not in {"LONG", "SHORT"}:
        raise XBRLDevelopmentError(f"invalid XBRL direction {direction!r}")
    data = signals.copy()
    data["decision_session"] = pd.to_datetime(data["decision_session"]).dt.date
    if set(data["direction"].astype(str)) != {direction}:
        raise XBRLDevelopmentError("candidate direction drifted inside XBRL tranche")
    ...
    primary_cost = XBRL_PRIMARY_COST_BPS[direction] / 10_000.0
    stress_cost = XBRL_STRESS_COST_BPS[direction] / 10_000.0
    session = (
    ...
    if session["fold"].isna().any():
        raise XBRLDevelopmentError("XBRL signal session is missing fold attribution")
    for fold in range(fold_count):
        group = session.loc[session["fold"] == fold, "primary_gross_return"]
        fold_values.append(None if group.empty else float(group.mean() - primary_cost))
    ...
    year_means = {
        ...
        if len(values) >= XBRL_MIN_YEAR_SIGNAL_SESSIONS
    }
...
def with_deflated_diagnostic(
    metrics: Mapping[str, XBRLTrancheMetrics],
) -> dict[str, XBRLTrancheMetrics]:
    sharpes = np.asarray(
    ...
    result: dict[str, XBRLTrancheMetrics] = {}
    ...
    *,
    ...
    return {
        ...
        "year_robustness": bool(
            ...
            and metrics.positive_year_fraction >= XBRL_MIN_POSITIVE_YEAR_FRACTION
        ),
        "session_concentration": bool(
            metrics.max_single_session_row_fraction is not None
            and metrics.max_single_session_row_fraction <= XBRL_MAX_SINGLE_SESSION_ROW_FRACTION
        ),
        "instrument_concentration": bool(
            metrics.max_single_instrument_row_fraction is not None
            and metrics.max_single_instrument_row_fraction <= XBRL_MAX_SINGLE_INSTRUMENT_ROW_FRACTION
        ),
    }


def selection_checks(metrics: XBRLTrancheMetrics) -> dict[str, bool]:
    return _stage_checks(
        metrics,
        min_event_rows=XBRL_SELECTION_MIN_EVENT_ROWS,
        min_signal_sessions=XBRL_SELECTION_MIN_SIGNAL_SESSIONS,
        min_unique_instruments=XBRL_SELECTION_MIN_UNIQUE_INSTRUMENTS,
        min_positive_folds=XBRL_SELECTION_MIN_POSITIVE_FOLDS,
    )


def internal_checks(metrics: XBRLTrancheMetrics) -> dict[str, bool]:
    return _stage_checks(
        metrics,
        min_event_rows=XBRL_INTERNAL_MIN_EVENT_ROWS,
        min_signal_sessions=XBRL_INTERNAL_MIN_SIGNAL_SESSIONS,
        min_unique_instruments=XBRL_INTERNAL_MIN_UNIQUE_INSTRUMENTS,
        min_positive_folds=XBRL_INTERNAL_MIN_POSITIVE_FOLDS,
    )
...
def protected_source_precheck(rows: pd.DataFrame, candidate_id: str) -> dict[str, object]:
    ...
    gates = {
        "min_event_rows": raw_rows >= XBRL_PROTECTED_MIN_EVENT_ROWS,
        "min_signal_sessions": signal_sessions >= XBRL_PROTECTED_MIN_SIGNAL_SESSIONS,
        "min_unique_instruments": unique_instruments >= XBRL_PROTECTED_MIN_UNIQUE_INSTRUMENTS,
    }
    return {
...
class XBRLDevelopmentStudy:
    ...
    def __init__(self, settings: AtlasSettings, *, progress_callback: Any | None = None) -> None:
        ...
        self.predictor_rows_path = self.derived_root / XBRL_PREDICTOR_ROWS_RELATIVE
        self.predictor_report_path = self.derived_root / XBRL_PREDICTOR_REPORT_RELATIVE
        self.root = self.derived_root / XBRL_DEVELOPMENT_ROOT_RELATIVE
    ...
    def _read_json(self, path: Path, label: str) -> dict[str, Any]:
        if not path.is_file():
            raise XBRLDevelopmentError(f"missing {label}: {path}")
        value = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(value, dict):
            raise XBRLDevelopmentError(f"{label} must be a JSON object")
        return value

    def _load_predictors(self) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
        report = self._read_json(self.predictor_report_path, "XBRL predictor report")
        if report.get("contract_version") != XBRL_PREDICTOR_CONTRACT:
            raise XBRLDevelopmentError("XBRL predictor contract mismatch")
        if report.get("scientific_fingerprint") != XBRL_SCIENTIFIC_FINGERPRINT:
            raise XBRLDevelopmentError("XBRL predictor scientific fingerprint mismatch")
        if int(report.get("target_outcome_rows_read", -1)) != 0:
            raise XBRLDevelopmentError("XBRL predictor artifact was not outcome-blind")
        if int(report.get("protected_return_rows_read", -1)) != 0:
            raise XBRLDevelopmentError("XBRL predictor artifact consumed protected returns")
        rows = _load_jsonl(self.predictor_rows_path)
        if sha256_file(self.predictor_rows_path) != str(report.get("predictor_rows_sha256") or ""):
            raise XBRLDevelopmentError("XBRL predictor rows SHA differs from source-only report")
        if len(rows) != int(report.get("predictor_rows", -1)):
            raise XBRLDevelopmentError("XBRL predictor row count differs from report")
        for row in rows:
            if row.get("scientific_fingerprint") != XBRL_SCIENTIFIC_FINGERPRINT:
                raise XBRLDevelopmentError("XBRL predictor row scientific fingerprint drifted")
            forbidden = _FORBIDDEN_PREDICTOR_FIELDS.intersection(row)
            if forbidden:
                raise XBRLDevelopmentError(
                    "XBRL predictor contains forbidden outcome fields: " + ", ".join(sorted(forbidden))
                )
        ...
        if development.empty:
            raise XBRLDevelopmentError("XBRL development predictor population is empty")
        development["decision_session"] = pd.to_datetime(development["decision_session"]).dt.date
        if development["decision_session"].max() > date.fromisoformat(XBRL_DEVELOPMENT_LAST_SIGNAL):
            raise XBRLDevelopmentError("XBRL development predictor crossed frozen boundary")
        if not protected.empty:
            protected["decision_session"] = pd.to_datetime(protected["decision_session"]).dt.date
            if protected["decision_session"].min() < date.fromisoformat(XBRL_PROTECTED_START):
                raise XBRLDevelopmentError("XBRL protected predictor crossed frozen start")
    ...
    def _split_evidence(self) -> tuple[pd.DataFrame, str]:
        ...
        if (
            ...
            raise XBRLDevelopmentError("accepted Phase26 split evidence is unavailable")
        if int(report.get("protected_return_reads", -1)) != 0:
            raise XBRLDevelopmentError("accepted split evidence is attached to consumed protected returns")
        ...
        if not path.is_file() or len(expected_sha) != 64 or sha256_file(path) != expected_sha:
            raise XBRLDevelopmentError("accepted split evidence SHA mismatch")
    ...
    def _boundaries_and_exits(
    ...
    ) -> tuple[XBRLDevelopmentBoundaries, pd.DataFrame, tuple[date, ...], tuple[date, ...]]:
        start = date.fromisoformat(XBRL_PERFORMANCE_SIGNAL_START)
        development_last = date.fromisoformat(XBRL_DEVELOPMENT_LAST_SIGNAL)
        outer_end = date.fromisoformat(XBRL_OUTER_EMBARGO_END)
        ...
        for decision in frame["decision_session"]:
            ...
            if index is None or index + XBRL_PRIMARY_HORIZON_SESSIONS >= len(full_grid):
                raise XBRLDevelopmentError(f"cannot resolve frozen 63-session exit for {decision}")
            exits.append(full_grid[index + XBRL_PRIMARY_HORIZON_SESSIONS])
        frame["exit_session"] = exits
        if frame["exit_session"].max() > outer_end:
            raise XBRLDevelopmentError("XBRL development outcome crossed outer embargo end")
    ...
    def _development_outcomes(
        ...
        try:
            ...
            con.register("xbrl_predictors", query)
            con.register("xbrl_splits", splits)
            result = con.execute(
                f"""
                WITH needed AS (
                    SELECT ticker AS symbol, CAST(decision_session AS DATE) AS session_date
                    FROM xbrl_predictors
                    UNION SELECT ticker, CAST(exit_session AS DATE) FROM xbrl_predictors
                    UNION SELECT 'SPY', CAST(decision_session AS DATE) FROM xbrl_predictors
                    UNION SELECT 'SPY', CAST(exit_session AS DATE) FROM xbrl_predictors
                ), bars AS (
        ...
        if len(result) != len(query):
            raise XBRLDevelopmentError(
                "XBRL exact outcome join cardinality drifted; duplicate daily keys suspected"
            )
        ...
        if bool(spy_missing.any()):
            raise XBRLDevelopmentError("SPY benchmark is missing an exact frozen entry/exit session")
        stock_missing = (
        ...
        if usable.empty:
            raise XBRLDevelopmentError("XBRL development outcomes are empty after path-quality censoring")
        ...
        direction = np.where(usable["direction"].astype(str).eq("LONG"), 1.0, -1.0)
        usable["primary_gross_return"] = direction * (
            usable["stock_return"] - usable["spy_return"]
        )
        usable["unhedged_gross_return"] = direction * usable["stock_return"]
        for field in (
            "stock_return",
            "spy_return",
            "primary_gross_return",
            ...
            if not np.isfinite(usable[field].to_numpy(dtype=float)).all():
                raise XBRLDevelopmentError(f"nonfinite XBRL development {field}")
        usable.insert(0, "outcome_contract_version", XBRL_OUTCOME_CONTRACT)
    ...
    def run(self) -> dict[str, Any]:
        if xbrl_scientific_fingerprint() != XBRL_SCIENTIFIC_FINGERPRINT:
            raise XBRLDevelopmentError("frozen XBRL scientific policy fingerprint drifted")
        if development_implementation_fingerprint() != XBRL_DEVELOPMENT_IMPLEMENTATION_FINGERPRINT:
            raise XBRLDevelopmentError("XBRL development implementation fingerprint drifted")
        if XBRL_PROTECTED_RETURNS_BEFORE_FINALISTS_ALLOWED:
            raise XBRLDevelopmentError("protected-return blindness contract drifted")
        if XBRL_RUNNER_UP_SUBSTITUTION_ALLOWED:
            raise XBRLDevelopmentError("runner-up substitution contract drifted")
        if XBRL_MULTIPLE_TESTING_METHOD != "HOLM_BONFERRONI_GLOBAL_6":
            raise XBRLDevelopmentError("XBRL multiplicity method drifted")
        if XBRL_SELECTION_WINNER_RULE != "highest_primary_selection_LCB_then_candidate_id":
            raise XBRLDevelopmentError("XBRL selection winner rule drifted")
        ...
        selection_mapping = _fold_mapping(selection_sessions, XBRL_SELECTION_FOLDS)
        internal_mapping = _fold_mapping(internal_sessions, XBRL_INTERNAL_VALIDATION_FOLDS)
        selection_metrics: dict[str, XBRLTrancheMetrics] = {}
        selection_gate_results: dict[str, dict[str, bool]] = {}
        directions = {spec.candidate_id: spec.direction for spec in XBRL_HYPOTHESES}
        for spec in XBRL_HYPOTHESES:
            candidate = outcomes.loc[
                outcomes["candidate_id"].astype(str).eq(spec.candidate_id)
                & outcomes["decision_session"].between(boundary.selection_start, boundary.selection_end)
            ].copy()
            metrics = tranche_metrics(
                candidate,
                direction=spec.direction,
                confidence=XBRL_SELECTION_CONFIDENCE,
                fold_mapping=selection_mapping,
                fold_count=XBRL_SELECTION_FOLDS,
        ...
        selection_passers = [
            ...
            if all(selection_gate_results[candidate_id].values())
        ...
        internal_metrics: dict[str, XBRLTrancheMetrics] = {}
        ...
        for candidate_id in winners:
            candidate = outcomes.loc[
                ...
                & outcomes["decision_session"].between(boundary.internal_start, boundary.internal_end)
            ].copy()
            metrics = tranche_metrics(
                candidate,
                direction=directions[candidate_id],
                confidence=XBRL_INTERNAL_CONFIDENCE,
                fold_mapping=internal_mapping,
                fold_count=XBRL_INTERNAL_VALIDATION_FOLDS,
        ...
        outcome_path = self.derived_root / XBRL_DEVELOPMENT_OUTCOMES_RELATIVE
        atomic_write_text(outcome_path, outcome_text)

        finalist_artifact = {
            "contract_version": XBRL_FINALIST_CONTRACT,
            "scientific_fingerprint": XBRL_SCIENTIFIC_FINGERPRINT,
        ...
        finalist_path = self.derived_root / XBRL_FINALISTS_RELATIVE
        ...
        report = {
            "contract_version": XBRL_DEVELOPMENT_CONTRACT,
            "scientific_fingerprint": XBRL_SCIENTIFIC_FINGERPRINT,
            "development_implementation_fingerprint": XBRL_DEVELOPMENT_IMPLEMENTATION_FINGERPRINT,
            "status": status,
            "pass": True,
            ...
            "predictor_source_reads_performed": int(
                predictor_report.get("source_reads_performed", 0)
            ),
            ...
            "next_scientific_action": (
                "If one or more finalists pass the frozen protected source-only precheck, "
                "open protected returns once for only those finalists; otherwise close this mechanism without support."
            ),
        }
        report_path = self.derived_root / XBRL_DEVELOPMENT_REPORT_RELATIVE
