...
class Phase25Gate6RecoveredIndependentValidator(Phase25Gate6IndependentValidator):
    """Reuse the accepted Gate6 validator with authoritative-recovery evidence."""
    ...
    def _validate_upstream_evidence(
        ...
        *,
        ...
        self._validate_recovery_binding(through_date=through_date, gate6=gate6)
    ...
    def _validate_recovered_reference_events(
        ...
        *,
        ...
        for raw_session in reacquired:
            ...
            elif mode in {"ALREADY_CURRENT", "NO_EXISTING_UNIVERSE_BUILD_REQUIRED"}:
    ...
    def run(self, *, through_date: date) -> dict[str, object]:
        ...
        gate6_path = (
            Phase25Gate6RecoveredPrerequisiteReconstruction(self.settings)
            .report_path(through_date)
        )
        ...
        if not isinstance(checks, dict):
            raise Phase25Gate6RecoveryError(
                "Gate6 independent-validation checks are malformed"
            )
        checks.update(
            {
