...
from .phase28_policy import (
    ...
    PHASE28_OUTCOME_HORIZON_SESSIONS,
    PHASE28_PRIMARY_COST_BPS,
    PHASE28_PROTECTED_CONFIDENCE,
    PHASE28_PROTECTED_FOLDS,
    PHASE28_PROTECTED_MIN_POSITIVE_FOLDS,
    PHASE28_PROTECTED_MIN_RAW_ROWS,
    PHASE28_PROTECTED_MIN_SIGNAL_SESSIONS,
...
PHASE28_CONFIRMATION_REPORT_CONTRACT_VERSION = (
    "phase28-confirmation-v1-frozen-finalist-tail-only-protected-outcomes"
)
PHASE28_PROTECTED_PREDICTION_CONTRACT_VERSION = (
    "phase28-protected-prediction-v1-no-outcomes"
)
PHASE28_PROTECTED_SCORE_SIGNAL_CONTRACT_VERSION = (
    "phase28-protected-score-signal-v1-frozen-before-outcome-read"
)
PHASE28_PROTECTED_SIGNAL_CONTRACT_VERSION = (
    "phase28-protected-signal-v1-finalist-tail-outcomes-only"
)
PHASE28_PROTECTED_READ_PLAN_CONTRACT_VERSION = (
    "phase28-protected-read-plan-v1-immutable-resumable-exact-signal-keys"
)
PHASE28_SUPPORT_OVERLAY_CONTRACT_VERSION = (
    "phase28-support-overlay-v1-historical-analytical-only"
)


class Phase28ConfirmationError(RuntimeError):
    pass


def _read_json(path: Path, label: str) -> dict[str, object]:
    if not path.is_file():
        raise Phase28ConfirmationError(f"missing {label}: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise Phase28ConfirmationError(f"invalid JSON for {label}: {path}") from exc
    if not isinstance(payload, dict):
        raise Phase28ConfirmationError(f"{label} must be a JSON object")
    return payload


def _read_parquet(path: Path, *, order_by: str) -> pd.DataFrame:
    if not path.is_file():
        raise Phase28ConfirmationError(f"missing parquet evidence: {path}")
...
def _write_parquet(
    ...
    *,
    ...
    con = connect_utc(":memory:")
    try:
        con.register("phase28_confirmation_write", frame)
        compression = settings.data.parquet.compression.upper()
        row_group_size = int(settings.data.parquet.row_group_size)
        con.execute(
            f"COPY (SELECT * FROM phase28_confirmation_write ORDER BY {order_by}) "
...
def protected_checks(metrics: Phase28TrancheMetrics) -> dict[str, bool]:
    return {
        "min_raw_rows": metrics.raw_rows >= PHASE28_PROTECTED_MIN_RAW_ROWS,
        "min_signal_sessions": metrics.signal_sessions >= PHASE28_PROTECTED_MIN_SIGNAL_SESSIONS,
        "positive_folds": metrics.positive_folds >= PHASE28_PROTECTED_MIN_POSITIVE_FOLDS,
...
def _candidate_by_id(candidate_id: str) -> Phase28CandidateSpec:
    try:
        return next(candidate for candidate in PHASE28_CANDIDATES if candidate.candidate_id == candidate_id)
    except StopIteration as exc:
        raise Phase28ConfirmationError(f"unknown Phase28 finalist candidate: {candidate_id}") from exc


class Phase28ProtectedConfirmation:
    """Confirm only frozen Phase28 finalist score-tail keys against the one-time holdout."""
    ...
    def __init__(self, settings: AtlasSettings) -> None:
        ...
        self.root = derived / "strategy_evaluation" / "phase28" / "v1" / "confirmation"
    ...
    def _finalists(self) -> tuple[tuple[dict[str, object], ...], Path]:
        ...
        payload = _read_json(path, "Phase28 frozen finalists")
        if payload.get("contract_version") != PHASE28_FINALIST_ARTIFACT_CONTRACT_VERSION:
            raise Phase28ConfirmationError("Phase28 finalist contract mismatch")
        if payload.get("phase28_policy_fingerprint") != phase28_policy_fingerprint():
            raise Phase28ConfirmationError("Phase28 finalist policy fingerprint mismatch")
        if payload.get("frozen") is not True:
            raise Phase28ConfirmationError("Phase28 finalist artifact is not frozen")
        if int(payload.get("protected_returns_read", -1)) != 0 or int(
            payload.get("protected_candidate_rows_read", -1)
        ) != 0:
            raise Phase28ConfirmationError("Phase28 finalists were not frozen protected-blind")
        entries = payload.get("finalists")
        if not isinstance(entries, list):
            raise Phase28ConfirmationError("Phase28 finalist entries are malformed")
        normalized: list[dict[str, object]] = []
        seen: set[str] = set()
        directions: set[str] = set()
        for entry in entries:
            if not isinstance(entry, dict):
                raise Phase28ConfirmationError("Phase28 finalist entry must be an object")
            candidate_id = str(entry.get("candidate_id") or "")
            candidate = _candidate_by_id(candidate_id)
            if candidate_id in seen or candidate.direction in directions:
                raise Phase28ConfirmationError("Phase28 finalist direction/cardinality invalid")
            if str(entry.get("direction")) != candidate.direction or str(entry.get("family")) != candidate.family:
                raise Phase28ConfirmationError("Phase28 finalist definition drift")
    ...
    def _blindness_report(self, finalist_path: Path) -> tuple[dict[str, object], Path]:
        ...
        payload = _read_json(path, "Phase28 protected blindness audit")
        if payload.get("contract_version") != PHASE28_BLINDNESS_AUDIT_CONTRACT_VERSION:
            raise Phase28ConfirmationError("Phase28 blindness audit contract mismatch")
        if payload.get("phase28_policy_fingerprint") != phase28_policy_fingerprint():
            raise Phase28ConfirmationError("Phase28 blindness policy fingerprint mismatch")
        if payload.get("pass") is not True or payload.get("eligible_for_phase28_protected_reuse") is not True:
            raise Phase28ConfirmationError("Phase28 holdout reuse was not independently approved")
        if payload.get("protected_holdout_consumed") is not False or int(
            payload.get("protected_returns_read", -1)
        ) != 0:
            raise Phase28ConfirmationError("Phase28 blindness audit is not pre-read")
        if payload.get("phase28_finalists_sha256") != sha256_file(finalist_path):
            raise Phase28ConfirmationError("Phase28 blindness audit finalist SHA mismatch")
        return payload, path

    def _protected_frame(self) -> tuple[pd.DataFrame, Path]:
        report = _read_json(self.population.report_path(), "Phase28 population report")
        if report.get("contract_version") != PHASE28_POPULATION_REPORT_CONTRACT_VERSION:
            raise Phase28ConfirmationError("Phase28 population report contract mismatch")
        if report.get("phase28_policy_fingerprint") != phase28_policy_fingerprint():
            raise Phase28ConfirmationError("Phase28 population policy fingerprint mismatch")
        if report.get("pass") is not True or int(report.get("protected_return_reads", -1)) != 0:
            raise Phase28ConfirmationError("Phase28 population is not protected-blind passing")
        path = self.population.protected_path()
        if report.get("protected_sha256") != sha256_file(path):
            raise Phase28ConfirmationError("Phase28 protected population SHA mismatch")
        frame = _read_parquet(path, order_by="as_of_date, instrument_id")
        if set(frame["phase28_contract_version"].astype(str)) != {
            PHASE28_PROTECTED_FRAME_CONTRACT_VERSION
        }:
            raise Phase28ConfirmationError("Phase28 protected row contract mismatch")
    ...
    def _score_finalists(
        ...
        *,
        ...
        for entry in finalist_entries:
            ...
            scored.insert(0, "prediction_contract_version", PHASE28_PROTECTED_PREDICTION_CONTRACT_VERSION)
            ...
            fired.insert(0, "score_signal_contract_version", PHASE28_PROTECTED_SCORE_SIGNAL_CONTRACT_VERSION)
        ...
        if predictions:
            ...
            combined_predictions.insert(0, "prediction_contract_version", pd.Series(dtype="string"))
            ...
            combined_predictions["phase28_score"] = pd.Series(dtype=float)
        if signals:
            combined_signals = pd.concat(signals, ignore_index=True, sort=False)
        else:
            combined_signals = combined_predictions.iloc[0:0].copy()
            combined_signals.insert(0, "score_signal_contract_version", pd.Series(dtype="string"))
    ...
    def _write_scored_artifacts(
        ...
        _write_parquet(
            self.settings, score_signals, score_signals_path, order_by="candidate_id, as_of_date, instrument_id"
        )
    ...
    def _future_query_keys(self, score_signals: pd.DataFrame) -> pd.DataFrame:
        ...
        if not splits.empty:
            for ticker, group in splits.groupby("ticker", sort=False, observed=True):
                split_dates[str(ticker)] = tuple(sorted(pd.to_datetime(group["execution_date"]).dt.date))

        result = score_signals.copy()
        result["as_of_date"] = pd.to_datetime(result["as_of_date"]).dt.date
        result["future_date"] = result["session_seq"].map(
            lambda value: future_by_seq.get(int(value) + PHASE28_OUTCOME_HORIZON_SESSIONS)
        )
        ...
        if not result.empty and max(result["future_date"]) > PHASE26_OUTCOME_EVIDENCE_END:
            raise Phase28ConfirmationError("Phase28 protected future date exceeds frozen evidence")
    ...
    def _read_plan_payload(
        ...
        *,
        ...
        return {
            "contract_version": PHASE28_PROTECTED_READ_PLAN_CONTRACT_VERSION,
            ...
            "protected_network_predictors_sha256": sha256_file(protected_path),
    ...
    def _ensure_read_plan(
        ...
        *,
        ...
        if path.is_file():
            existing = _read_json(path, "Phase28 protected read plan")
            ...
            if any(existing.get(key) != expected.get(key) for key in stable_fields):
                raise Phase28ConfirmationError("Phase28 protected read plan changed after commitment")
    ...
    def _join_outcomes(self, query_keys: pd.DataFrame) -> pd.DataFrame:
        ...
        if result.duplicated(["candidate_id", "as_of_date", "instrument_id"], keep=False).any():
            raise Phase28ConfirmationError("Phase28 protected outcome join is non-unique")
        ...
        result["primary_net_return"] = result["directional_return"] - PHASE28_PRIMARY_COST_BPS / 10_000.0
        result["stress_net_return"] = result["directional_return"] - PHASE28_STRESS_COST_BPS / 10_000.0
        result.insert(0, "signal_contract_version", PHASE28_PROTECTED_SIGNAL_CONTRACT_VERSION)
        return result
    ...
    def _write_support(
        ...
        *,
        ...
        payload: dict[str, object] = {
            "contract_version": PHASE28_SUPPORT_OVERLAY_CONTRACT_VERSION,
            "phase28_policy_fingerprint": phase28_policy_fingerprint(),
            "authority": "HISTORICAL_ANALYTICAL_STRATEGY_SUPPORT_ONLY",
            "supported_candidate_ids": list(confirmed_ids),
            "candidate_definitions": [
                asdict(_candidate_by_id(candidate_id)) for candidate_id in confirmed_ids
            ],
            "finalists_sha256": sha256_file(finalist_path),
    ...
    def run(self) -> dict[str, object]:
        if self.report_path().is_file():
            existing = _read_json(self.report_path(), "Phase28 protected confirmation")
            if existing.get("contract_version") != PHASE28_CONFIRMATION_REPORT_CONTRACT_VERSION:
                raise Phase28ConfirmationError("existing Phase28 confirmation contract mismatch")
            if existing.get("phase28_policy_fingerprint") != phase28_policy_fingerprint():
                raise Phase28ConfirmationError("existing Phase28 confirmation policy mismatch")
            if existing.get("pass") is not True:
                raise Phase28ConfirmationError("existing Phase28 confirmation is not passing")
            return existing
        ...
        for entry in finalist_entries:
            ...
            rows = _assign_fold(rows, field="protected_fold", desired_folds=PHASE28_PROTECTED_FOLDS)
            item = tranche_metrics(
                rows,
                predictions=pd.DataFrame(),
                confidence=PHASE28_PROTECTED_CONFIDENCE,
