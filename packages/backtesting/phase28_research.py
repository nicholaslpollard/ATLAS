...
from .phase28_policy import (
    ...
    PHASE28_PURGE_SESSIONS,
    PHASE28_RESEARCH_START,
    PHASE28_RUNNER_UP_SUBSTITUTION_ALLOWED,
    PHASE28_SELECTION_CONFIDENCE,
    PHASE28_SELECTION_FOLDS,
    PHASE28_SELECTION_FRACTION,
...
PHASE28_RESEARCH_REPORT_CONTRACT_VERSION = (
    "phase28-research-v1-fixed-network-signals-holm-internal-protected-blind"
)
PHASE28_PREDICTION_ARTIFACT_CONTRACT_VERSION = "phase28-development-prediction-v1"
PHASE28_SIGNAL_ARTIFACT_CONTRACT_VERSION = "phase28-development-signal-v1"
PHASE28_FINALIST_ARTIFACT_CONTRACT_VERSION = (
    "phase28-finalists-v1-selection-internal-fixed-network-protected-unread"
)


class Phase28ResearchError(RuntimeError):
    pass


@dataclass(frozen=True, slots=True)
class Phase28DevelopmentBoundaries:
...
def chronological_boundaries(sessions: Iterable[date]) -> Phase28DevelopmentBoundaries:
    ordered = tuple(sorted(set(sessions)))
    if len(ordered) < 20:
        raise Phase28ResearchError("too few Phase28 development sessions")
    selection_count = int(math.floor(len(ordered) * PHASE28_SELECTION_FRACTION))
    internal_offset = selection_count + PHASE28_PURGE_SESSIONS
    if selection_count <= 0 or internal_offset >= len(ordered):
        raise Phase28ResearchError("invalid Phase28 development split")
    selection = ordered[:selection_count]
    purge = ordered[selection_count:internal_offset]
    internal = ordered[internal_offset:]
    if len(purge) != PHASE28_PURGE_SESSIONS or not internal:
        raise Phase28ResearchError("Phase28 selection/internal purge partition is incomplete")
    return Phase28DevelopmentBoundaries(
...
def _derived_seed(label: str) -> int:
    ...
    return PHASE28_BOOTSTRAP_SEED + int(digest[:8], 16)


def _bootstrap(values: np.ndarray, *, confidence: float, label: str) -> tuple[float, float]:
    if values.ndim != 1 or len(values) == 0:
        raise Phase28ResearchError("bootstrap requires a nonempty session vector")
    n = len(values)
    block = min(PHASE28_BOOTSTRAP_BLOCK_SESSIONS, n)
    block_count = int(math.ceil(n / block))
    rng = np.random.default_rng(_derived_seed(label))
    starts = rng.integers(0, n, size=(PHASE28_BOOTSTRAP_REPLICATES, block_count))
    offsets = np.arange(block, dtype=np.int64)
    indices = ((starts[:, :, None] + offsets) % n).reshape(
        PHASE28_BOOTSTRAP_REPLICATES, -1
    )[:, :n]
...
def _eligible_state_means(
    ...
    *,
    ...
    for state, subset in grouped.groupby(state_field, sort=True, observed=True):
        if subset["as_of_date"].nunique() < PHASE28_MIN_REGIME_SIGNAL_SESSIONS:
            continue
...
def _mean_session_spearman(frame: pd.DataFrame) -> float | None:
    if frame.empty or "phase28_score" not in frame.columns:
        return None
    correlations: list[float] = []
    for _, group in frame.groupby("as_of_date", sort=True, observed=True):
        if len(group) < 3:
            continue
        scores = pd.to_numeric(group["phase28_score"], errors="coerce").rank(method="average")
...
def _fold_economic_means(
    ...
    *,
    ...
    if mapping.duplicated(["as_of_date"], keep=False).any():
        raise Phase28ResearchError("Phase28 signal session belongs to multiple folds")
    merged = session.merge(mapping, on="as_of_date", how="left", validate="one_to_one")
    if merged[fold_field].isna().any():
        raise Phase28ResearchError("Phase28 signal session is missing fold attribution")
    result: list[float] = []
...
def tranche_metrics(
    ...
    *,
    ...
    fold_field: str,
    label: str,
) -> Phase28TrancheMetrics:
    if signals.empty:
        return Phase28TrancheMetrics(
    ...
    primary_cost = PHASE28_PRIMARY_COST_BPS / 10_000.0
    stress_cost = PHASE28_STRESS_COST_BPS / 10_000.0
    session = (
    ...
    year_means = {
        ...
        if len(values) >= PHASE28_MIN_YEAR_SIGNAL_SESSIONS
    }
    ...
    cost_means = {
        ...
        for cost in PHASE28_COST_GRID_BPS
    }
...
def selection_checks(metrics: Phase28TrancheMetrics) -> dict[str, bool]:
    return {
        "min_raw_rows": metrics.raw_rows >= PHASE28_SELECTION_MIN_RAW_ROWS,
        "min_signal_sessions": metrics.signal_sessions >= PHASE28_SELECTION_MIN_SIGNAL_SESSIONS,
        "positive_folds": metrics.positive_folds >= PHASE28_SELECTION_MIN_POSITIVE_FOLDS,
        ...
        "year_robustness": bool(
            ...
            and metrics.positive_year_fraction >= PHASE28_MIN_POSITIVE_YEAR_FRACTION
        ),
        "market_state_robustness": bool(
            metrics.positive_market_state_fraction is not None
            and metrics.positive_market_state_fraction >= PHASE28_MIN_POSITIVE_REGIME_FRACTION
        ),
        "ticker_state_robustness": bool(
            metrics.positive_ticker_state_fraction is not None
            and metrics.positive_ticker_state_fraction >= PHASE28_MIN_POSITIVE_REGIME_FRACTION
        ),
        "session_concentration": bool(
            metrics.max_single_session_row_fraction is not None
            and metrics.max_single_session_row_fraction <= PHASE28_MAX_SINGLE_SESSION_ROW_FRACTION
        ),
    }


def internal_checks(metrics: Phase28TrancheMetrics) -> dict[str, bool]:
    return {
        "min_raw_rows": metrics.raw_rows >= PHASE28_INTERNAL_MIN_RAW_ROWS,
        "min_signal_sessions": metrics.signal_sessions >= PHASE28_INTERNAL_MIN_SIGNAL_SESSIONS,
        "positive_folds": metrics.positive_folds >= PHASE28_INTERNAL_MIN_POSITIVE_FOLDS,
        "primary_mean_positive": bool(metrics.primary_mean_return is not None and metrics.primary_mean_return > 0),
        "primary_lcb_positive": bool(metrics.primary_lcb is not None and metrics.primary_lcb > 0),
        "stress_mean_positive": bool(metrics.stress_mean_return is not None and metrics.stress_mean_return > 0),
        "session_concentration": bool(
...
def _deflated_sharpe_diagnostics(
    metrics: Mapping[str, Phase28TrancheMetrics],
...
def _with_deflated(
    metrics: dict[str, Phase28TrancheMetrics]
) -> dict[str, Phase28TrancheMetrics]:
    diagnostic = _deflated_sharpe_diagnostics(metrics)
    return {
        key: Phase28TrancheMetrics(
            **{
                **item.to_dict(),
...
def _fold_mapping(sessions: tuple[date, ...], folds: int) -> dict[date, int]:
    if len(sessions) < folds:
        raise Phase28ResearchError("too few Phase28 sessions for fold attribution")
    blocks = [
        tuple(block.tolist())
...
def _assign_fold(frame: pd.DataFrame, *, mapping: Mapping[date, int], field: str) -> pd.DataFrame:
    ...
    if result[field].isna().any():
        raise Phase28ResearchError("Phase28 fold attribution is incomplete")
...
def candidate_predictions(frame: pd.DataFrame, candidate: Phase28CandidateSpec) -> pd.DataFrame:
    ...
    raw = pd.to_numeric(result[candidate.family], errors="coerce").to_numpy(dtype=float)
    if not np.isfinite(raw).all():
        raise Phase28ResearchError(f"non-finite frozen signal for {candidate.candidate_id}")
    multiplier = 1.0 if candidate.direction == "LONG" else -1.0
    result["phase28_score"] = raw * multiplier
    return result
...
class Phase28DevelopmentResearch:
    def __init__(self, settings: AtlasSettings) -> None:
        ...
        self.root = derived / "strategy_evaluation" / "phase28" / "v1" / "research"
    ...
    def _load_development(self) -> tuple[pd.DataFrame, dict[str, object], Path]:
        ...
        if not isinstance(report, dict):
            raise Phase28ResearchError("Phase28 population report is missing or malformed")
        if report.get("contract_version") != PHASE28_POPULATION_REPORT_CONTRACT_VERSION:
            raise Phase28ResearchError("Phase28 population report contract mismatch")
        if report.get("phase28_policy_fingerprint") != phase28_policy_fingerprint():
            raise Phase28ResearchError("Phase28 population policy fingerprint mismatch")
        if report.get("pass") is not True or int(report.get("protected_return_reads", -1)) != 0:
            raise Phase28ResearchError("Phase28 population is not protected-blind passing")
        path = self.population.development_path()
        if not path.is_file() or report.get("development_sha256") != sha256_file(path):
            raise Phase28ResearchError("Phase28 development population SHA mismatch")
        con = connect_utc(":memory:")
        try:
            frame = con.execute(
            ...
            con.close()
        if frame.empty:
            raise Phase28ResearchError("Phase28 development population is empty")
        if set(frame["phase28_contract_version"].astype(str)) != {
            PHASE28_DEVELOPMENT_FRAME_CONTRACT_VERSION
        }:
            raise Phase28ResearchError("Phase28 development row contract mismatch")
        frame["as_of_date"] = pd.to_datetime(frame["as_of_date"]).dt.date
        return frame, report, report_path

    def _boundaries(self, frame: pd.DataFrame) -> Phase28DevelopmentBoundaries:
        sessions = tuple(sorted(set(frame["as_of_date"])))
        accepted = set(
            self.calendar.sessions_in_range(
                date.fromisoformat(PHASE28_RESEARCH_START),
                date.fromisoformat(PHASE28_DEVELOPMENT_END),
            )
        )
        if not sessions or not set(sessions).issubset(accepted):
            raise Phase28ResearchError("Phase28 development sessions drift outside frozen calendar")
        return chronological_boundaries(sessions)

    @staticmethod
    def _decorate_predictions(
        frame: pd.DataFrame, candidate: Phase28CandidateSpec, *, stage: str
    ) -> pd.DataFrame:
        result = frame.copy()
        result.insert(0, "prediction_contract_version", PHASE28_PREDICTION_ARTIFACT_CONTRACT_VERSION)
    ...
    def _decorate_signals(
        ...
        result = frame.copy()
        result.insert(0, "signal_contract_version", PHASE28_SIGNAL_ARTIFACT_CONTRACT_VERSION)
        result.insert(1, "candidate_id", candidate.candidate_id)
        result.insert(2, "candidate_family", candidate.family)
        result.insert(3, "strategy_direction", candidate.direction)
        result.insert(4, "research_stage", stage)
        result["primary_net_return"] = (
            ...
            - PHASE28_PRIMARY_COST_BPS / 10_000.0
        )
        result["stress_net_return"] = (
            pd.to_numeric(result["directional_return"], errors="coerce")
            - PHASE28_STRESS_COST_BPS / 10_000.0
        )
        return result

    def run(self) -> dict[str, object]:
        if PHASE28_RUNNER_UP_SUBSTITUTION_ALLOWED:
            raise Phase28ResearchError("Phase28 runner-up substitution must remain disabled")
        ...
        if selection_frame.empty or internal_frame.empty:
            raise Phase28ResearchError("Phase28 selection/internal population is empty")
        ...
        selection_fold_map = _fold_mapping(selection_sessions, PHASE28_SELECTION_FOLDS)
        internal_fold_map = _fold_mapping(internal_sessions, PHASE28_INTERNAL_VALIDATION_FOLDS)

        prediction_artifacts: list[pd.DataFrame] = []
        signal_artifacts: list[pd.DataFrame] = []
        selection_metrics: dict[str, Phase28TrancheMetrics] = {}
        ...
        for candidate in PHASE28_CANDIDATES:
            ...
            signals = _assign_fold(
                select_fixed_tail(predictions),
                mapping=selection_fold_map,
                field="selection_fold",
            )
            decorated_predictions = self._decorate_predictions(
                predictions, candidate, stage="SELECTION"
            )
            decorated_signals = self._decorate_signals(
                signals, candidate, stage="SELECTION"
            )
            ...
            metrics = tranche_metrics(
                ...
                confidence=PHASE28_SELECTION_CONFIDENCE,
        ...
        if len(winner_ids) > 2 * PHASE28_MAX_SELECTION_WINNERS_PER_DIRECTION:
            raise Phase28ResearchError("Phase28 selected more than one winner per direction")

        internal_metrics: dict[str, Phase28TrancheMetrics] = {}
        internal_check_map: dict[str, dict[str, bool]] = {}
        finalist_ids: list[str] = []
        for candidate_id in winner_ids:
            candidate = next(item for item in PHASE28_CANDIDATES if item.candidate_id == candidate_id)
            predictions = _assign_fold(
            ...
            decorated_predictions = self._decorate_predictions(
                predictions, candidate, stage="INTERNAL_VALIDATION"
            )
            decorated_signals = self._decorate_signals(
                signals, candidate, stage="INTERNAL_VALIDATION"
            )
            prediction_artifacts.append(decorated_predictions)
            signal_artifacts.append(decorated_signals)
            metrics = tranche_metrics(
                decorated_signals,
                predictions=decorated_predictions,
                confidence=PHASE28_INTERNAL_CONFIDENCE,
        ...
        if len(finalist_ids) > 2:
            raise Phase28ResearchError("Phase28 created more than one finalist per direction")
        ...
        finalist_payload: dict[str, object] = {
            "contract_version": PHASE28_FINALIST_ARTIFACT_CONTRACT_VERSION,
        ...
        checks = {
            ...
            "exact_candidate_count": len(PHASE28_CANDIDATES) == 8,
            ...
            "runner_up_substitution_disabled": not PHASE28_RUNNER_UP_SUBSTITUTION_ALLOWED,
        ...
        if not all(checks.values()):
            ...
            raise Phase28ResearchError("Phase28 research checks failed: " + ", ".join(failed))
        ...
        report: dict[str, object] = {
            ...
            "candidate_count": len(PHASE28_CANDIDATES),
