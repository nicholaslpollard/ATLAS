...
from .phase29_policy import (
    ...
    PHASE29_CANDIDATES,
    PHASE29_COST_GRID_BPS,
    PHASE29_DEVELOPMENT_END,
    PHASE29_INTERNAL_CONFIDENCE,
    PHASE29_INTERNAL_MIN_POSITIVE_FOLDS,
    PHASE29_INTERNAL_MIN_RAW_ROWS,
    PHASE29_INTERNAL_MIN_SIGNAL_SESSIONS,
    PHASE29_INTERNAL_VALIDATION_FOLDS,
    PHASE29_MAX_SELECTION_WINNERS_PER_DIRECTION,
    ...
    PHASE29_PURGE_SESSIONS,
    PHASE29_RESEARCH_START,
    PHASE29_RUNNER_UP_SUBSTITUTION_ALLOWED,
    PHASE29_SELECTION_CONFIDENCE,
    PHASE29_SELECTION_FOLDS,
    PHASE29_SELECTION_FRACTION,
...
PHASE29_RESEARCH_REPORT_CONTRACT_VERSION = (
    "phase29-research-v1-fixed-relative-value-holm-internal-protected-blind"
)
PHASE29_PREDICTION_ARTIFACT_CONTRACT_VERSION = "phase29-development-prediction-v1"
PHASE29_SIGNAL_ARTIFACT_CONTRACT_VERSION = "phase29-development-signal-v1"
PHASE29_FINALIST_ARTIFACT_CONTRACT_VERSION = (
    "phase29-finalists-v1-selection-internal-relative-value-protected-unread"
)


class Phase29ResearchError(RuntimeError):
    pass


@dataclass(frozen=True, slots=True)
class Phase29DevelopmentBoundaries:
...
def chronological_boundaries(sessions: Iterable[date]) -> Phase29DevelopmentBoundaries:
    ordered = tuple(sorted(set(sessions)))
    if len(ordered) < 20:
        raise Phase29ResearchError("too few Phase29 development sessions")
    selection_count = int(math.floor(len(ordered) * PHASE29_SELECTION_FRACTION))
    internal_offset = selection_count + PHASE29_PURGE_SESSIONS
    if selection_count <= 0 or internal_offset >= len(ordered):
        raise Phase29ResearchError("invalid Phase29 development split")
    selection = ordered[:selection_count]
    purge = ordered[selection_count:internal_offset]
    internal = ordered[internal_offset:]
    if len(purge) != PHASE29_PURGE_SESSIONS or not internal:
        raise Phase29ResearchError("Phase29 selection/internal purge partition is incomplete")
    return Phase29DevelopmentBoundaries(
...
def _bootstrap(values: np.ndarray, *, confidence: float, label: str) -> tuple[float, float]:
    if values.ndim != 1 or len(values) == 0:
        raise Phase29ResearchError("bootstrap requires a nonempty session vector")
    n = len(values)
    ...
    starts = rng.integers(0, n, size=(PHASE29_BOOTSTRAP_REPLICATES, block_count))
    offsets = np.arange(block, dtype=np.int64)
    indices = ((starts[:, :, None] + offsets) % n).reshape(
        PHASE29_BOOTSTRAP_REPLICATES, -1
    )[:, :n]
...
def _eligible_state_means(
    ...
    for state, subset in grouped.groupby(state_field, sort=True, observed=True):
        if subset["as_of_date"].nunique() < PHASE29_MIN_REGIME_SIGNAL_SESSIONS:
            continue
        result[str(state)] = float(
            pd.to_numeric(subset["directional_return"], errors="coerce").mean() - primary_cost
        )
    return result


def _mean_session_spearman(frame: pd.DataFrame) -> float | None:
    if frame.empty or "phase29_score" not in frame.columns:
        return None
    values: list[float] = []
    for _, group in frame.groupby("as_of_date", sort=True, observed=True):
        if len(group) < 3:
            continue
        scores = pd.to_numeric(group["phase29_score"], errors="coerce").rank(method="average")
        returns = pd.to_numeric(group["directional_return"], errors="coerce").rank(method="average")
...
def _fold_economic_means(
    session: pd.DataFrame, signals: pd.DataFrame, *, fold_field: str, primary_cost: float
) -> tuple[float, ...]:
    ...
    if mapping.duplicated(["as_of_date"], keep=False).any():
        raise Phase29ResearchError("Phase29 signal session belongs to multiple folds")
    merged = session.merge(mapping, on="as_of_date", how="left", validate="one_to_one")
    if merged[fold_field].isna().any():
        raise Phase29ResearchError("Phase29 signal session is missing fold attribution")
    return tuple(
    ...
    *,
    ...
    if data.empty:
        return tranche_metrics(
            data, predictions=predictions, confidence=confidence, fold_field=fold_field, label=label
        )
    ...
    market_means = _eligible_state_means(data, state_field="market_state", primary_cost=primary_cost)
    ...
    cost_means = {
        ...
        for cost in PHASE29_COST_GRID_BPS
    }
    ...
    if not prediction_data.empty:
        prediction_data["as_of_date"] = pd.to_datetime(prediction_data["as_of_date"]).dt.date
        comparator_session = (
            prediction_data.groupby("as_of_date", sort=True, observed=True)["directional_return"]
            .mean()
            .to_numpy(dtype=float)
        )
        broad_comparator = (
            None if len(comparator_session) == 0 else float(np.mean(comparator_session - primary_cost))
        )
    else:
...
def selection_checks(metrics: Phase29TrancheMetrics) -> dict[str, bool]:
    return {
        "min_raw_rows": metrics.raw_rows >= PHASE29_SELECTION_MIN_RAW_ROWS,
        "min_signal_sessions": metrics.signal_sessions >= PHASE29_SELECTION_MIN_SIGNAL_SESSIONS,
        "positive_folds": metrics.positive_folds >= PHASE29_SELECTION_MIN_POSITIVE_FOLDS,
        ...
        "year_robustness": bool(
            ...
            and metrics.positive_year_fraction >= PHASE29_MIN_POSITIVE_YEAR_FRACTION
        ),
        "market_state_robustness": bool(
            metrics.positive_market_state_fraction is not None
            and metrics.positive_market_state_fraction >= PHASE29_MIN_POSITIVE_REGIME_FRACTION
        ),
        "ticker_state_robustness": bool(
            metrics.positive_ticker_state_fraction is not None
            and metrics.positive_ticker_state_fraction >= PHASE29_MIN_POSITIVE_REGIME_FRACTION
        ),
...
def internal_checks(metrics: Phase29TrancheMetrics) -> dict[str, bool]:
    return {
        "min_raw_rows": metrics.raw_rows >= PHASE29_INTERNAL_MIN_RAW_ROWS,
        "min_signal_sessions": metrics.signal_sessions >= PHASE29_INTERNAL_MIN_SIGNAL_SESSIONS,
        "positive_folds": metrics.positive_folds >= PHASE29_INTERNAL_MIN_POSITIVE_FOLDS,
        "primary_mean_positive": bool(metrics.primary_mean_return is not None and metrics.primary_mean_return > 0),
        "primary_lcb_positive": bool(metrics.primary_lcb is not None and metrics.primary_lcb > 0),
        "stress_mean_positive": bool(metrics.stress_mean_return is not None and metrics.stress_mean_return > 0),
        "session_concentration": bool(
...
def holm_bonferroni(
    p_values: Mapping[str, float], *, alpha: float = PHASE29_MULTIPLE_TESTING_ALPHA
...
def _with_deflated(metrics: dict[str, Phase29TrancheMetrics]) -> dict[str, Phase29TrancheMetrics]:
    ...
    result: dict[str, Phase29TrancheMetrics] = {}
    for key, item in metrics.items():
        ...
        result[key] = Phase29TrancheMetrics(
            **{
                **item.to_dict(),
...
def _fold_mapping(sessions: tuple[date, ...], folds: int) -> dict[date, int]:
    if len(sessions) < folds:
        raise Phase29ResearchError("too few Phase29 sessions for fold attribution")
...
def _assign_fold(frame: pd.DataFrame, *, mapping: Mapping[date, int], field: str) -> pd.DataFrame:
    ...
    if result[field].isna().any():
        raise Phase29ResearchError("Phase29 fold attribution is incomplete")
    result[field] = result[field].astype(int)
    return result


def candidate_predictions(frame: pd.DataFrame, candidate: Phase29CandidateSpec) -> pd.DataFrame:
    label = "bullish" if candidate.direction == "LONG" else "bearish"
    result = frame.loc[frame["direction"].astype(str) == label].copy()
    raw = pd.to_numeric(result[candidate.raw_signal_field], errors="coerce").to_numpy(dtype=float)
    if not np.isfinite(raw).all():
        raise Phase29ResearchError(f"nonfinite frozen signal for {candidate.candidate_id}")
    result["phase29_score"] = raw * float(candidate.orientation)
    return result
...
class Phase29DevelopmentResearch:
    def __init__(self, settings: AtlasSettings) -> None:
        ...
        self.root = derived / "strategy_evaluation" / "phase29" / "v1" / "research"
    ...
    def _load_development(self) -> tuple[pd.DataFrame, dict[str, object], Path]:
        ...
        if not report_path.is_file():
            raise Phase29ResearchError("Phase29 population report is missing")
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if not isinstance(report, dict):
            raise Phase29ResearchError("Phase29 population report is malformed")
        if report.get("contract_version") != PHASE29_POPULATION_REPORT_CONTRACT_VERSION:
            raise Phase29ResearchError("Phase29 population contract mismatch")
        if report.get("phase29_policy_fingerprint") != phase29_policy_fingerprint():
            raise Phase29ResearchError("Phase29 population policy mismatch")
        if report.get("pass") is not True or int(report.get("protected_return_reads", -1)) != 0:
            raise Phase29ResearchError("Phase29 population is not protected-blind passing")
        path = self.population.development_path()
        if not path.is_file() or report.get("development_sha256") != sha256_file(path):
            raise Phase29ResearchError("Phase29 development population SHA mismatch")
        con = connect_utc(":memory:")
        try:
            frame = con.execute(
            ...
            con.close()
        if frame.empty:
            raise Phase29ResearchError("Phase29 development population is empty")
        if set(frame["phase29_contract_version"].astype(str)) != {
            PHASE29_DEVELOPMENT_FRAME_CONTRACT_VERSION
        }:
            raise Phase29ResearchError("Phase29 development row contract mismatch")
        frame["as_of_date"] = pd.to_datetime(frame["as_of_date"]).dt.date
        return frame, report, report_path

    def _boundaries(self, frame: pd.DataFrame) -> Phase29DevelopmentBoundaries:
        sessions = tuple(sorted(set(frame["as_of_date"])))
        accepted = set(
            self.calendar.sessions_in_range(
                date.fromisoformat(PHASE29_RESEARCH_START), date.fromisoformat(PHASE29_DEVELOPMENT_END)
            )
        )
        if not sessions or not set(sessions).issubset(accepted):
            raise Phase29ResearchError("Phase29 development sessions drift outside frozen calendar")
        return chronological_boundaries(sessions)

    @staticmethod
    def _decorate(frame: pd.DataFrame, candidate: Phase29CandidateSpec, *, stage: str, signal: bool) -> pd.DataFrame:
        result = frame.copy()
        contract = (
            PHASE29_SIGNAL_ARTIFACT_CONTRACT_VERSION
            if signal
            else PHASE29_PREDICTION_ARTIFACT_CONTRACT_VERSION
        )
    ...
    def run(self) -> dict[str, object]:
        ...
        selection_frame = frame.loc[
            (frame["as_of_date"] >= boundaries.selection_start)
            & (frame["as_of_date"] <= boundaries.selection_end)
        ].copy()
        internal_frame = frame.loc[
            (frame["as_of_date"] >= boundaries.internal_start)
            & (frame["as_of_date"] <= boundaries.internal_end)
        ].copy()
        selection_sessions = tuple(sorted(set(selection_frame["as_of_date"])))
        internal_sessions = tuple(sorted(set(internal_frame["as_of_date"])))
        selection_fold_map = _fold_mapping(selection_sessions, PHASE29_SELECTION_FOLDS)
        internal_fold_map = _fold_mapping(internal_sessions, PHASE29_INTERNAL_VALIDATION_FOLDS)

        prediction_artifacts: list[pd.DataFrame] = []
        signal_artifacts: list[pd.DataFrame] = []
        selection_metrics: dict[str, Phase29TrancheMetrics] = {}
        selection_check_map: dict[str, dict[str, bool]] = {}
        for candidate in PHASE29_CANDIDATES:
            predictions = _assign_fold(
                candidate_predictions(selection_frame, candidate),
                mapping=selection_fold_map,
                field="selection_fold",
            )
            signals = _assign_fold(
                select_fixed_tail(predictions), mapping=selection_fold_map, field="selection_fold"
            )
            decorated_predictions = self._decorate(
                predictions, candidate, stage="SELECTION", signal=False
            )
            decorated_signals = self._decorate(signals, candidate, stage="SELECTION", signal=True)
        ...
        if len(holm) != len(PHASE29_CANDIDATES):
            raise Phase29ResearchError("Phase29 Holm family is incomplete")
        survivors = {
            candidate.candidate_id
            for candidate in PHASE29_CANDIDATES
        ...
        for direction in ("LONG", "SHORT"):
            ...
            eligible.sort(
                key=lambda candidate: (
                    -float(selection_metrics[candidate.candidate_id].primary_lcb or -math.inf),
                    -float(selection_metrics[candidate.candidate_id].primary_mean_return or -math.inf),
                    candidate.candidate_id,
                )
            )
            if eligible:
                winner_ids.append(eligible[0].candidate_id)
        if len(winner_ids) > 2 * PHASE29_MAX_SELECTION_WINNERS_PER_DIRECTION:
            raise Phase29ResearchError("Phase29 selected more than one winner per direction")

        internal_metrics: dict[str, Phase29TrancheMetrics] = {}
        internal_check_map: dict[str, dict[str, bool]] = {}
        finalist_ids: list[str] = []
        for candidate_id in winner_ids:
            candidate = next(item for item in PHASE29_CANDIDATES if item.candidate_id == candidate_id)
            predictions = _assign_fold(
                candidate_predictions(internal_frame, candidate),
                mapping=internal_fold_map,
                field="internal_fold",
            )
            signals = _assign_fold(
                select_fixed_tail(predictions), mapping=internal_fold_map, field="internal_fold"
            )
            decorated_predictions = self._decorate(
                predictions, candidate, stage="INTERNAL_VALIDATION", signal=False
            )
            decorated_signals = self._decorate(
                signals, candidate, stage="INTERNAL_VALIDATION", signal=True
            )
            prediction_artifacts.append(decorated_predictions)
            signal_artifacts.append(decorated_signals)
            metrics = tranche_metrics(
                decorated_signals,
                predictions=decorated_predictions,
                confidence=PHASE29_INTERNAL_CONFIDENCE,
        ...
        if len(finalist_ids) > 2:
            raise Phase29ResearchError("Phase29 created more than one finalist per direction")
            ...
            finalist_entries.append(
                {
                    ...
                    "raw_signal_field": candidate.raw_signal_field,
        ...
        finalist_payload: dict[str, object] = {
            "contract_version": PHASE29_FINALIST_ARTIFACT_CONTRACT_VERSION,
        ...
        atomic_write_text(finalists_path, json.dumps(finalist_payload, indent=2, sort_keys=True) + "\n")

        checks = {
            "population_report_pass": population_report.get("pass") is True,
            "protected_returns_unread": int(population_report.get("protected_return_reads", -1)) == 0,
            "exact_candidate_count": len(PHASE29_CANDIDATES) == 4,
            ...
            "runner_up_substitution_disabled": not PHASE29_RUNNER_UP_SUBSTITUTION_ALLOWED,
            "finalist_artifact_protected_unread": int(finalist_payload["protected_returns_read"]) == 0,
            "external_activity_zero": True,
        }
        if not all(checks.values()):
            failed = sorted(name for name, passed in checks.items() if not passed)
            raise Phase29ResearchError("Phase29 research checks failed: " + ", ".join(failed))
