...
from packages.data.historical_option_reference_v3_contract import (
    ...
    HISTORICAL_OPTION_REFERENCE_V3_CONTRACT,
...
class HistoricalOptionReferenceV3Error(RuntimeError):
    pass
    ...
    for attempt in range(1, int(cfg.max_attempts) + 1):
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "ATLAS-historical-option-reference-v3/1",
            },
            method="GET",
        )
        try:
    ...
    *,
    ...
    if not differing or not differing.issubset(allowed):
        raise HistoricalOptionReferenceV3Error(
            ...
            f"{sorted(differing)} exceed frozen V3 allowance {sorted(allowed)}"
        )
    ...
    if len(expirations) != 1:
        raise HistoricalOptionReferenceV3Error(
            f"{partition.key}: {ticker}: conflict rows disagree on expiration"
        )
    ...
    overview_json = _stable_json(overview_rows[0])
    current_by_json = {
        _stable_json(row): row
        for row in highest_rows
    }
    if overview_json not in current_by_json:
        raise HistoricalOptionReferenceV3Error(
            f"{partition.key}: {ticker}: stable pre-expiration Contract Overview "
            "did not exactly match any current conflicting payload"
        )

    selected = current_by_json[overview_json]
    lineage = {
...
def _resolve_ticker_versions(
    ...
    *,
    ...
    normalized: dict[str, object] = {
        ...
        "conflict_resolution_request_ids_json": _stable_json(
            conflict_lineage["request_ids"]
            if conflict_lineage is not None
            else []
        ),
    }
        ...
        if row_count:
            con.execute(
                ...
                COPY (
                    SELECT
                        ...
                        CAST(conflict_resolution_request_ids_json AS VARCHAR)
                            AS conflict_resolution_request_ids_json
                    FROM read_json_auto(
            ...
            con.execute(
                ...
                CREATE TABLE empty_option_reference (
                    ...
                    conflict_resolution_request_ids_json VARCHAR
                )
                """
            )
            con.execute(
...
def _partition_paths(
    ...
    expiration_month = partition.expiration_gte.replace(day=1)
    suffix = (
        Path("massive/historical_option_reference_v3")
        / f"reference_as_of={REFERENCE_AS_OF_DATE.isoformat()}"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    ...
    manifest_suffix = (
        Path("massive")
        / "historical_option_reference_v3"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    )
...
def _acquire_partition(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v3") / partition.key
    )
...
def _rebuild_partition_from_parent_raw(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v3_parent_reuse")
        / partition.key
    )
    staging.mkdir(parents=True, exist_ok=True)
    normalized_jsonl = staging / "contracts.normalized.jsonl.tmp"
...
def run_historical_option_reference_v3_acquisition(
    settings: AtlasSettings,
    *,
    workers: int = 4,
) -> dict[str, object]:
    ...
    reused_v3: list[dict[str, object]] = []
    ...
    print(
        "historical option reference v3: "
        f"{len(partitions)} monthly partitions / "
        f"{len(reused_v3)} verified V3 reusable / "
    ...
    completed: list[dict[str, object]] = list(reused_v3)

    if rebuild_from_v2:
        def rebuild_v2_task(
            item: tuple[ReferencePartition, dict[str, object], Path],
        ) -> dict[str, object]:
            partition, v2_receipt, raw_path = item
            return _rebuild_partition_from_parent_raw(
                settings,
                partition,
                parent_receipt=v2_receipt,
                raw_path=raw_path,
        ...
        def rebuild_v2_progress(
            item: tuple[ReferencePartition, dict[str, object], Path],
            receipt: dict[str, object],
            done: int,
        ) -> None:
            partition, _receipt, _raw = item
            storage = inspect_research_storage(settings)
            print(
                f"  {done}/{len(rebuild_from_v2)} rebuilt {partition.key} "
                "from verified V2 raw: "
        ...
        completed.extend(
            _run_bounded(
                rebuild_from_v2,
                workers=workers,
                task=rebuild_v2_task,
                on_complete=rebuild_v2_progress,
            )
        )
    ...
    if rebuild_from_v1:
        ...
        def rebuild_v1_progress(
            item: tuple[ReferencePartition, dict[str, object]],
            receipt: dict[str, object],
            done: int,
        ) -> None:
            partition, _receipt = item
            storage = inspect_research_storage(settings)
            print(
        ...
        completed.extend(
            _run_bounded(
                rebuild_from_v1,
                workers=workers,
                task=rebuild_v1_task,
                on_complete=rebuild_v1_progress,
            )
        )
    ...
    if len(completed) != len(partitions):
        raise HistoricalOptionReferenceV3Error(
            f"V3 partition completion mismatch: {len(completed)} != {len(partitions)}"
        )
    ...
    corpus_basis = {
        "contract_fingerprint": HISTORICAL_OPTION_REFERENCE_V3_CONTRACT_FINGERPRINT,
        "reference_as_of_date": REFERENCE_AS_OF_DATE.isoformat(),
    ...
    summary: dict[str, object] = {
        ...
        "acquired_partitions_this_run": (
            len(rebuild_from_v2) + len(rebuild_from_v1) + len(provider_pending)
        ),
        "raw_provider_records": total_raw_records,
        "normalized_unique_contracts": total_normalized,
    ...
    summary_path = settings.resolved_path(
        ...
        "historical_option_reference_v3_summary.json"
    )
