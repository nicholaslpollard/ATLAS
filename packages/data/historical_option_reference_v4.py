...
from packages.data.historical_option_reference_v4_contract import (
    ...
    HISTORICAL_OPTION_REFERENCE_V4_CONTRACT,
...
class HistoricalOptionReferenceV4Error(RuntimeError):
    pass
    ...
    *,
    ...
    if not differing or not differing.issubset(allowed):
        raise HistoricalOptionReferenceV4Error(
            ...
            f"{sorted(differing)} exceed frozen V4 allowance {sorted(allowed)}"
        )
...
def _partition_paths(
    ...
    expiration_month = partition.expiration_gte.replace(day=1)
    suffix = (
        Path("massive/historical_option_reference_v4")
        / f"reference_as_of={REFERENCE_AS_OF_DATE.isoformat()}"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    ...
    manifest_suffix = (
        Path("massive")
        / "historical_option_reference_v4"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    )
...
def _acquire_partition(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v4") / partition.key
    )
    ...
    try:
        ...
        receipt: dict[str, object] = {
            "status": "COMPLETE",
            "contract": HISTORICAL_OPTION_REFERENCE_V4_CONTRACT,
...
def _rebuild_partition_from_parent_raw(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v4_parent_reuse")
        / partition.key
    )
    staging.mkdir(parents=True, exist_ok=True)
    normalized_jsonl = staging / "contracts.normalized.jsonl.tmp"
...
def run_historical_option_reference_v4_acquisition(
    settings: AtlasSettings,
    *,
    workers: int = 5,
) -> dict[str, object]:
    ...
    reused_v4: list[dict[str, object]] = []
    ...
    print(
        "historical option reference v4: "
        f"{len(partitions)} monthly partitions / "
        f"{len(reused_v4)} verified V4 reusable / "
    ...
    completed: list[dict[str, object]] = list(reused_v4)
    ...
    if len(completed) != len(partitions):
        raise HistoricalOptionReferenceV4Error(
            f"V4 partition completion mismatch: {len(completed)} != {len(partitions)}"
        )
    ...
    corpus_basis = {
        "contract_fingerprint": HISTORICAL_OPTION_REFERENCE_V4_CONTRACT_FINGERPRINT,
        "reference_as_of_date": REFERENCE_AS_OF_DATE.isoformat(),
    ...
    summary: dict[str, object] = {
        ...
        "reused_verified_partitions": len(reused_v4),
