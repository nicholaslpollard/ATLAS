...
from packages.data.historical_option_reference_v5_contract import (
    ...
    HISTORICAL_OPTION_REFERENCE_V5_CONTRACT,
...
class HistoricalOptionReferenceV5Error(RuntimeError):
    pass
...
def _resolve_massive_api_key(settings: AtlasSettings) -> str:
    ...
    if not key:
        raise HistoricalOptionReferenceV5Error(
    ...
    *,
    ...
    if len(target_rows) != 1:
        raise HistoricalOptionReferenceV5Error(
            ...
            f"target row, received {len(target_rows)}"
        )
    ...
    *,
    ...
    if not differing or not differing.issubset(allowed):
        raise HistoricalOptionReferenceV5Error(
            ...
            f"{sorted(differing)} exceed frozen V5 allowance {sorted(allowed)}"
        )
    ...
    if len(highest_payloads) != 1:
        if settings is None or api_key is None:
            raise HistoricalOptionReferenceV5Error(
                ...
                f"highest correction rank {highest_rank}; V5 resolver context missing"
            )
        if highest_rank == -1:
...
def _partition_paths(
    ...
    expiration_month = partition.expiration_gte.replace(day=1)
    suffix = (
        Path("massive/historical_option_reference_v5")
        / f"reference_as_of={REFERENCE_AS_OF_DATE.isoformat()}"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    ...
    manifest_suffix = (
        Path("massive")
        / "historical_option_reference_v5"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    )
...
def _acquire_partition(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v5") / partition.key
    )
...
def _rebuild_partition_from_parent_raw(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v5_parent_reuse")
        / partition.key
    )
    staging.mkdir(parents=True, exist_ok=True)
    normalized_jsonl = staging / "contracts.normalized.jsonl.tmp"
    ...
    try:
        ...
        if receipt["raw_sha256"] != parent_receipt.get("raw_sha256"):
            raise HistoricalOptionReferenceV5Error(
                f"{partition.key}: parent raw hash changed during local V5 rebuild"
            )
        receipt["receipt_fingerprint"] = _stable_hash(receipt)
        _atomic_write_json(paths["receipt"], receipt)
        return receipt
...
def run_historical_option_reference_v5_acquisition(
    settings: AtlasSettings,
    *,
    workers: int = 5,
) -> dict[str, object]:
    ...
    reused_v5: list[dict[str, object]] = []
    ...
    print(
        "historical option reference v5: "
        f"{len(partitions)} monthly partitions / "
        f"{len(reused_v5)} verified V5 reusable / "
    ...
    completed: list[dict[str, object]] = list(reused_v5)
    ...
    if len(completed) != len(partitions):
        raise HistoricalOptionReferenceV5Error(
            f"V5 partition completion mismatch: {len(completed)} != {len(partitions)}"
        )
    ...
    corpus_basis = {
        "contract_fingerprint": HISTORICAL_OPTION_REFERENCE_V5_CONTRACT_FINGERPRINT,
        "reference_as_of_date": REFERENCE_AS_OF_DATE.isoformat(),
    ...
    summary: dict[str, object] = {
        ...
        "reused_verified_partitions": len(reused_v5),
    ...
    summary_path = settings.resolved_path(
        ...
        "historical_option_reference_v5_summary.json"
    )
