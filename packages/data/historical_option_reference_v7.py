...
import packages.data.historical_option_reference_v6 as v6_parent
...
from packages.data.historical_option_reference_v7_contract import (
    ...
    HISTORICAL_OPTION_REFERENCE_V7_CONTRACT,
    ...
    UNVERSIONED_DELIVERABLE_CFI_ALLOWED_DIFFERING_FIELDS,
...
class HistoricalOptionReferenceV7Error(RuntimeError):
    pass
...
class HistoricalOptionReferenceV7Quarantine(RuntimeError):
    ...
    for attempt in range(1, int(cfg.max_attempts) + 1):
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "ATLAS-historical-option-reference-v7/1",
            },
            method="GET",
        )
        try:
...
def _historical_target_row_once(
    ...
    *,
    ...
    if len(target_rows) != 1:
        raise HistoricalOptionReferenceV7HistoricalTargetCardinalityError(
            ticker=ticker,
            as_of=as_of,
...
def _cash_additional_underlying_usd(row: dict[str, Any]) -> Decimal | None:
    value = row.get("additional_underlyings")
    if not isinstance(value, list) or len(value) != 1:
        return None
    item = value[0]
    if not isinstance(item, dict):
        return None
    if str(item.get("type") or "").lower() != "currency":
        return None
    if str(item.get("underlying") or "").upper() != "USD":
        return None
    try:
...
def _resolve_narrow_unversioned_conflict(
    ...
    *,
    ...
    identity_allowed = set(CONFLICT_ALLOWED_DIFFERING_FIELDS)
    deliverable_cfi_required = set(UNVERSIONED_DELIVERABLE_CFI_ALLOWED_DIFFERING_FIELDS)

    resolution_branch: str
    resolution_policy: str
    diagnostic_fingerprints: list[str] = []

    if differing and differing.issubset(identity_allowed):
    ...
    elif differing == deliverable_cfi_required:
        resolution_branch = "UNVERSIONED_CASH_DELIVERABLE_CLASSIFICATION"
        resolution_policy = UNVERSIONED_DELIVERABLE_CFI_POLICY
        if len(highest_rows) != 2:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification branch "
                f"requires exactly 2 conflicting rows; observed={len(highest_rows)}"
            )
        underlyings = {str(row.get("underlying_ticker") or "") for row in highest_rows}
        exchanges = {str(row.get("primary_exchange") or "") for row in highest_rows}
        shares = {str(row.get("shares_per_contract") or "") for row in highest_rows}
        if len(underlyings) != 1 or "" in underlyings:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification rows "
                "must share one non-empty underlying ticker"
            )
        if len(exchanges) != 1:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification rows "
                "must share one primary exchange"
            )
        if len(shares) != 1:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification rows "
                "must share one shares-per-contract value"
            )
        cash_rows = [
            row for row in highest_rows
            if _cash_additional_underlying_usd(row) is not None
        ]
        no_deliverable_rows = [
            row for row in highest_rows
            if not row.get("additional_underlyings")
        ]
        cfi_values = {str(row.get("cfi") or "") for row in highest_rows}
        if len(cash_rows) != 1 or len(no_deliverable_rows) != 1:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification branch "
                "requires exactly one positive USD cash deliverable row and one row "
                "without additional underlyings"
            )
        if len(cfi_values) != 2 or "" in cfi_values:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: cash-deliverable/classification branch "
                "requires two distinct non-empty CFI values"
            )
        if ticker == "O:ARTC140719C00025000":
            diagnostic_fingerprints.append(ARTC_DIAGNOSTIC_EVIDENCE_FINGERPRINT)
    else:
        raise HistoricalOptionReferenceV7Error(
            f"{partition.key}: {ticker}: same-rank conflict fields "
            f"{sorted(differing)} are outside frozen V7 branches; "
            f"identity_allowance={sorted(identity_allowed)}; "
            f"cash_deliverable_classification_required={sorted(deliverable_cfi_required)}"
        )
    ...
    shares_per_contract = {item[4] for item in structural}
    if (
        len(expirations) != 1
        or len(contract_types) != 1
        or len(strike_prices) != 1
        or len(shares_per_contract) != 1
    ):
        raise HistoricalOptionReferenceV7Error(
    ...
    if resolution_branch == "UNVERSIONED_CASH_DELIVERABLE_CLASSIFICATION":
        cash_amount = _cash_additional_underlying_usd(selected)
        if cash_amount is None:
            raise HistoricalOptionReferenceV7Error(
                f"{partition.key}: {ticker}: historical exact-match payload did not "
                "carry the required positive USD cash additional underlying"
            )
    else:
        cash_amount = None
    ...
    if cash_amount is not None:
        lineage["selected_cash_additional_underlying_usd"] = str(cash_amount)
        lineage["selected_cfi"] = str(selected.get("cfi") or "")
...
def _confirm_unresolved_primary_exchange_quarantine(
    ...
    *,
    ...
    settings: AtlasSettings,
    api_key: str,
    first_gap: HistoricalOptionReferenceV7HistoricalTargetCardinalityError,
) -> dict[str, object]:
    if partition.state != "EXPIRED":
        raise first_gap
    ...
    except HistoricalOptionReferenceV7HistoricalTargetCardinalityError as second_gap:
        if (
    ...
    *,
    ...
    if len(highest_payloads) != 1:
        ...
        if highest_rank == -1:
            try:
            ...
            except HistoricalOptionReferenceV7HistoricalTargetCardinalityError as gap:
                ...
                raise HistoricalOptionReferenceV7Quarantine(quarantine_record) from gap
        else:
...
def _partition_paths(
    ...
    expiration_month = partition.expiration_gte.replace(day=1)
    suffix = (
        Path("massive/historical_option_reference_v7")
        / f"reference_as_of={REFERENCE_AS_OF_DATE.isoformat()}"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    ...
    manifest_suffix = (
        Path("massive")
        / "historical_option_reference_v7"
        / f"state={partition.state.lower()}"
        / f"expiration_year={expiration_month.year:04d}"
        / f"expiration_month={expiration_month.month:02d}"
    )
    ...
    for spec in KNOWN_CONFLICTS:
        ...
        while next_url:
            if next_url in seen_urls:
                raise HistoricalOptionReferenceV7Error(
                    f"{ticker}: known-conflict preflight pagination loop"
                )
            ...
            if candidate in (None, ""):
                ...
                raise HistoricalOptionReferenceV7Error(
                    f"{ticker}: known-conflict preflight next_url was not a string"
                )
            else:
                next_url = candidate
        ...
        expected_hash = spec.get("expected_selected_provider_record_sha256")
        if expected_hash and normalized["provider_record_sha256"] != str(expected_hash):
            raise HistoricalOptionReferenceV7Error(
                f"{ticker}: known-conflict selected provider row hash changed"
            )
        expected_cfi = spec.get("expected_selected_cfi")
        if expected_cfi and normalized["cfi"] != str(expected_cfi):
            raise HistoricalOptionReferenceV7Error(
                f"{ticker}: known-conflict selected CFI changed: {normalized['cfi']!r}"
            )
        expected_cash = spec.get("expected_cash_additional_underlying_usd")
        if expected_cash is not None:
            selected_additional = json.loads(
                str(normalized["additional_underlyings_json"])
            )
            selected_row = {"additional_underlyings": selected_additional}
            observed_cash = _cash_additional_underlying_usd(selected_row)
            if observed_cash != Decimal(str(expected_cash)):
                raise HistoricalOptionReferenceV7Error(
                    f"{ticker}: known-conflict selected cash deliverable changed: "
                    f"{observed_cash!r}"
                )
        ...
        results.append(
            {
                ...
                "current_list_page_count": page_count,
                ...
                "selected_cfi": normalized["cfi"],
                "selected_additional_underlyings_json": normalized[
...
def _known_quarantine_probes(
    ...
    *,
    ...
    for spec in KNOWN_QUARANTINES:
        ...
        except HistoricalOptionReferenceV7Quarantine as quarantine:
...
def _verified_v6_raw_receipt(
    settings: AtlasSettings,
    partition: ReferencePartition,
) -> tuple[dict[str, object], Path] | None:
    receipt = v6_parent._verified_existing_receipt(settings, partition)
    if receipt is None:
        return None
    raw_path = v6_parent._receipt_data_path(settings, receipt.get("raw_path"))
...
def _acquire_partition(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v7") / partition.key
    )
    ...
    try:
        ...
        receipt: dict[str, object] = {
            "status": "COMPLETE",
            "contract": HISTORICAL_OPTION_REFERENCE_V7_CONTRACT,
...
def _rebuild_partition_from_parent_raw(
    ...
    *,
    ...
    staging = settings.resolved_path(
        Path("data/staging/historical_option_reference_v7_parent_reuse")
        / partition.key
    )
    staging.mkdir(parents=True, exist_ok=True)
    normalized_jsonl = staging / "contracts.normalized.jsonl.tmp"
...
def run_historical_option_reference_v7_acquisition(
    settings: AtlasSettings,
    *,
    workers: int = 5,
) -> dict[str, object]:
    ...
    for partition in partitions:
        ...
        if v6_source is not None:
            v6_receipt, raw_path = v6_source
            rebuild_from_v6.append((partition, v6_receipt, raw_path))
            continue
    ...
    corpus_basis = {
        "contract_fingerprint": HISTORICAL_OPTION_REFERENCE_V7_CONTRACT_FINGERPRINT,
        "reference_as_of_date": REFERENCE_AS_OF_DATE.isoformat(),
