...
class MarketDataPaths:
    ...
    def recurrent_exit_plan_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return root / "simulation" / "recurrent_exit_plan" / "current.json"

    def recurrent_close_evidence_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return root / "simulation" / "recurrent_close" / "current.json"
