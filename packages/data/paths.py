...
class MarketDataPaths:
    ...
    def recurrent_reserve_evidence_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return root / "simulation" / "recurrent_reserve" / "current.json"

    def recurrent_entry_evidence_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return root / "simulation" / "recurrent_entry" / "current.json"

    def recurrent_decision_stock_exit_plan_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return (
            root
            / "simulation"
            / "recurrent_decision_stock_exit_plan"
            / "current.json"
        )

    def recurrent_decision_stock_close_evidence_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return (
            root
            / "simulation"
            / "recurrent_decision_stock_close"
            / "current.json"
        )

    def recurrent_forecast_horizon_clock_book_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return (
            root
            / "simulation"
            / "recurrent_forecast_horizon_clock_book"
            / "current.json"
        )
    ...
    def recurrent_price_time_exit_resolution_file(self) -> Path:
        root = self.settings.resolved_path(self.settings.data.paths.live)
        return (
            root
            / "simulation"
            / "recurrent_price_time_exit_resolution"
            / "current.json"
        )
