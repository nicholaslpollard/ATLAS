...
LIFECYCLE_EXIT_FILL_CONTRACT_VERSION = str(
    LIFECYCLE_EXIT_FILL_CONTRACT["contract_id"]
)
_TOLERANCE = 1e-9


class LifecycleExitFillError(ValueError):
    pass
...
def _require_aware(value: datetime, *, label: str) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise LifecycleExitFillError(f"{label} must be timezone-aware")
...
class LifecycleExitFillInputsV1:
    fill_source_id: str
    ...
    def __post_init__(self) -> None:
        if not self.fill_source_id.strip():
            raise LifecycleExitFillError(
                "lifecycle exit-fill source id cannot be blank"
            )
        _require_fingerprint(
            self.fill_source_fingerprint,
            label="lifecycle exit-fill source fingerprint",
        )
        _require_aware(
            self.exited_utc,
            label="lifecycle exit-fill timestamp",
        )
        if (
...
@dataclass(frozen=True)
class LifecycleExitFillEvidenceV1:
    ...
    def __post_init__(self) -> None:
        if self.contract_version != LIFECYCLE_EXIT_FILL_CONTRACT_VERSION:
            raise LifecycleExitFillError(
                "lifecycle exit-fill contract version mismatch"
            )
        if (
        ...
        _require_aware(self.opened_utc, label="lifecycle position opened timestamp")
        _require_aware(self.exited_utc, label="lifecycle exit-fill timestamp")
        ...
        if not self.instrument_id.strip() or not self.ticker.strip():
            raise LifecycleExitFillError(
                "lifecycle exit underlying identity cannot be blank"
            )
        if not self.candidate_identifier.strip():
            raise LifecycleExitFillError(
                "lifecycle exit candidate identifier cannot be blank"
            )
        if not self.fill_source_id.strip():
            raise LifecycleExitFillError(
                "lifecycle exit source id cannot be blank"
            )
        if not math.isfinite(self.quantity) or self.quantity <= 0.0:
            raise LifecycleExitFillError(
                "lifecycle exit quantity must be finite and positive"
            )
        if (
            ...
            raise LifecycleExitFillError(
                "lifecycle exit multiplier must be finite and positive"
            )
        if (
            not math.isfinite(self.exit_price_per_unit)
            or self.exit_price_per_unit < 0.0
            ...
            raise LifecycleExitFillError(
                "lifecycle exit price must be finite and nonnegative"
            )
        if (
            ...
            raise LifecycleExitFillError(
                "lifecycle exit fees must be finite and nonnegative"
            )
        ...
        if not self.full_close:
            raise LifecycleExitFillError(
                "lifecycle exit-fill v1 must close the full position"
            )
        if not self.reason_codes:
            raise LifecycleExitFillError(
                "lifecycle exit-fill evidence requires reason codes"
            )
        if not self.descriptive_only:
            raise LifecycleExitFillError(
                "lifecycle exit-fill evidence must remain descriptive only"
            )

        if self.instrument_kind == InstrumentKind.STOCK:
            if self.quantity_unit != "SHARES":
                raise LifecycleExitFillError(
                    "stock lifecycle exit quantity unit must be SHARES"
                )
            if not _same(self.contract_multiplier, 1.0):
                raise LifecycleExitFillError(
                    "stock lifecycle exit multiplier must equal one"
                )
            if (
                ...
                raise LifecycleExitFillError(
                    "stock lifecycle exit cannot carry option lineage"
                )
        elif self.instrument_kind == InstrumentKind.OPTION:
            if self.quantity_unit != "CONTRACTS":
                raise LifecycleExitFillError(
                    "option lifecycle exit quantity unit must be CONTRACTS"
                )
            if not _same(self.quantity, round(self.quantity)):
                raise LifecycleExitFillError(
                    "option lifecycle exit quantity must be integral contracts"
                )
            if (
                ...
                raise LifecycleExitFillError(
                    "option lifecycle exit requires complete option lineage"
                )
        else:
            raise LifecycleExitFillError(
                "unsupported lifecycle exit instrument kind"
            )
        ...
        if any(forbidden):
            raise LifecycleExitFillError(
                "lifecycle exit-fill evidence cannot grant mutation, P&L, provider, "
                "broker, order, trading, promotion, or confluence authority"
            )

    @property
    def exit_fill_fingerprint(self) -> str:
        return lifecycle_exit_fill_fingerprint(self)


def lifecycle_exit_fill_fingerprint(
...
def _active_position(
    source_state: LifecyclePositionAccountStateV1,
    position_fingerprint: str,
    ...
    if len(matches) != 1:
        raise LifecycleExitFillError(
            "exact active lifecycle open position is required"
        )
    return matches[0]


def build_lifecycle_exit_fill_evidence(
    *,
    source_state: LifecyclePositionAccountStateV1,
    position_fingerprint: str,
    ...
    if (
        ...
        raise LifecycleExitFillError(
            "source lifecycle position account contract fingerprint mismatch"
        )
    if (
        source_state.state_fingerprint
        != lifecycle_position_account_state_fingerprint(source_state)
    ):
        raise LifecycleExitFillError(
    ...
    if inputs.exited_utc < source_state.as_of_utc:
        raise LifecycleExitFillError(
            "exit-fill timestamp cannot precede lifecycle position state"
        )
    ...
    return LifecycleExitFillEvidenceV1(
        contract_version=LIFECYCLE_EXIT_FILL_CONTRACT_VERSION,
        contract_fingerprint=LIFECYCLE_EXIT_FILL_CONTRACT_FINGERPRINT,
        source_position_contract_fingerprint=(
            LIFECYCLE_POSITION_ACCOUNT_CONTRACT_FINGERPRINT
        ),
        source_position_state_fingerprint=source_state.state_fingerprint,
