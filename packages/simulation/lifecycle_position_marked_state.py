...
LIFECYCLE_POSITION_MARKED_ACCOUNT_CONTRACT_VERSION = str(
    LIFECYCLE_POSITION_MARKED_ACCOUNT_CONTRACT["contract_id"]
)
_TOLERANCE = 1e-9


class LifecyclePositionMarkedAccountError(ValueError):
    pass
...
def _require_fingerprint(value: str, *, label: str) -> None:
    if len(value) != 64:
        raise LifecyclePositionMarkedAccountError(
...
class LifecyclePositionMarkedAccountStateV1:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            != LIFECYCLE_POSITION_MARKED_ACCOUNT_CONTRACT_VERSION
        ):
            raise LifecyclePositionMarkedAccountError(
                "lifecycle position marked contract version mismatch"
            )
        if (
            ...
            raise LifecyclePositionMarkedAccountError(
                "lifecycle position marked contract fingerprint mismatch"
            )
        if (
        ...
        _require_fingerprint(
            ...
            label="source lifecycle position state fingerprint",
        )
        _require_aware(
            self.valuation_utc,
            label="lifecycle post-reentry valuation timestamp",
        )
        ...
        if not self.complete_mark_coverage:
            raise LifecyclePositionMarkedAccountError(
                "accepted lifecycle position valuation requires complete marks"
            )
        if (
            ...
            raise LifecyclePositionMarkedAccountError(
                "lifecycle position valuation must identify mark-to-market computations"
            )
        ...
        if any(
            ...
            raise LifecyclePositionMarkedAccountError(
                "all marked positions must share the valuation timestamp"
            )

        if not _same(
            self.marked_open_position_value_dollars,
            sum(x.marked_value_dollars for x in self.marked_positions),
        ):
            raise LifecyclePositionMarkedAccountError(
        ...
        if not _same(
            ...
            sum(x.unrealized_pnl_dollars for x in self.marked_positions),
        ):
            raise LifecyclePositionMarkedAccountError(
        ...
        if not _same(self.account_book_equity, expected_book):
            raise LifecyclePositionMarkedAccountError(
                "book equity must preserve lifecycle fee and realized-P&L semantics"
            )
        if not _same(
            self.marked_equity,
        ...
        if any(forbidden):
            raise LifecyclePositionMarkedAccountError(
                "lifecycle position valuation cannot grant mutation, realized-P&L, "
                "exit, provider, broker, order, trading, promotion, or confluence authority"
            )

    @property
    def state_fingerprint(self) -> str:
        return lifecycle_position_marked_account_state_fingerprint(self)


def lifecycle_position_marked_account_state_fingerprint(
    state: LifecyclePositionMarkedAccountStateV1,
) -> str:
...
def _validate_mark(
    *,
    ...
    if (
        ...
        raise LifecyclePositionMarkedAccountError(
            "mark does not bind exact lifecycle open position"
        )
    pairs = (
        (
    ...
    if mark.valuation_utc != valuation_utc:
        raise LifecyclePositionMarkedAccountError(
            "all marks must share requested lifecycle valuation timestamp"
        )
    if mark.market_timestamp_utc < position.opened_utc:
        raise LifecyclePositionMarkedAccountError(
            "market mark cannot predate lifecycle position open"
        )
    if (
        ...
        raise LifecyclePositionMarkedAccountError(
            "stale or ineligible mark cannot create lifecycle valuation"
        )
...
def _mark_position(
    *,
    ...
    return MarkedOpenPositionV1(
        ...
        reason_codes=(
            "EXACT_LIFECYCLE_POST_REENTRY_POSITION_AND_FRESH_MARK_BOUND",
            ...
            "CLOSED_TRADES_NOT_REVALUED",
            "OPTION_ENTRY_DELTA_REMAINS_REFERENCE_ONLY",
        ),
    )


def build_lifecycle_position_marked_account_state(
    *,
    source_state: LifecyclePositionAccountStateV1,
    marks: Sequence[SimulatedMarketMarkEvidence],
    valuation_utc: datetime,
) -> LifecyclePositionMarkedAccountStateV1:
    if (
        source_state.contract_fingerprint
        != LIFECYCLE_POSITION_ACCOUNT_CONTRACT_FINGERPRINT
    ):
        raise LifecyclePositionMarkedAccountError(
            "lifecycle position state contract fingerprint mismatch"
        )
    if (
        ...
        raise LifecyclePositionMarkedAccountError(
            "lifecycle position state fingerprint mismatch"
        )
    _require_aware(
        valuation_utc,
        label="requested lifecycle post-reentry valuation timestamp",
    )
    if valuation_utc < source_state.as_of_utc:
        raise LifecyclePositionMarkedAccountError(
            "valuation timestamp cannot predate lifecycle position account state"
        )
    ...
    for mark in marks:
        ...
        if key in mark_by_decision:
            raise LifecyclePositionMarkedAccountError(
                "one lifecycle open position cannot receive multiple marks"
            )
    ...
    if provided_ids != expected_ids:
        ...
        raise LifecyclePositionMarkedAccountError(
            "complete lifecycle open-position mark coverage required; "
    ...
    return LifecyclePositionMarkedAccountStateV1(
        contract_version=LIFECYCLE_POSITION_MARKED_ACCOUNT_CONTRACT_VERSION,
