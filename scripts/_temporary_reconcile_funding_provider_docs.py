...
fp = "f76d77ebbf138924a22813773ad27276b0fa71691ddff1d21040171c7b6d3821"

provider_section = r'''## 2026-09-16 — Live market-data provider and transport policy
...
funding_section = f'''## 2026-09-16 — Explicit simulation funding/collateral terms

Track A now freezes the funding boundary under contract `{fp}`
...
old = '''The next bounded Track A package must freeze explicit funding/collateral semantics
...
new = f'''Explicit funding/collateral semantics are now frozen under contract `{fp}`
...
if old not in text:
    raise SystemExit("README fill continuation paragraph not found")
text = text.replace(old, new, 1)
marker = "## A33/B33 reference foundation"
if marker not in text:
    raise SystemExit("README insertion marker not found")
if "## 2026-09-16 — Live market-data provider and transport policy" not in text:
    text = text.replace(marker, provider_section + "\n" + funding_section + "\n" + marker, 1)
readme.write_text(text, encoding="utf-8")

roadmap = Path("docs/roadmap.md")
text = roadmap.read_text(encoding="utf-8")
old = '''- **Massive:** primary broad-market/reference provider where entitlement and PIT
  semantics are proven. Current known plan: Stocks Starter.'''
new = '''- **Canonical Parquet/DuckDB V2 lake:** primary broad historical/research source and
...
if old not in text:
    raise SystemExit("roadmap Massive provider bullet not found")
text = text.replace(old, new, 1)
old = '''- **Webull:** primary PAPER/sandbox and intended primary LIVE broker only after
  separate acceptance.'''
new = '''- **Webull:** primary PAPER/sandbox and intended primary LIVE broker only after
...
if old not in text:
    raise SystemExit("roadmap Webull provider bullet not found")
text = text.replace(old, new, 1)
old = '''- **Alpaca:** explicit/manual secondary broker. No automatic broker failover.'''
new = '''- **Alpaca execution:** explicit/manual secondary execution broker. Market-data
  primacy does not create execution authority. No automatic broker failover.'''
if old not in text:
    raise SystemExit("roadmap Alpaca broker bullet not found")
text = text.replace(old, new, 1)

immediate_old = '''Immediate Track A continuation after acceptance:
...
immediate_new = '''Immediate Track A continuation after acceptance:
...
5. leave mark-to-market, unrealized/realized P&L, exits/closeout and PAPER/LIVE broker
   authority to separately accepted packages.'''
if immediate_old not in text:
    raise SystemExit("roadmap funding continuation block not found")
text = text.replace(immediate_old, immediate_new, 1)

marker = "## 5. Accepted foundation through Phase32"
if marker not in text:
    raise SystemExit("roadmap provider insertion marker not found")
if "### Live/current market-data transport policy — 2026-09-16" not in text:
    detailed = provider_section.replace(
