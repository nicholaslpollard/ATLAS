...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the finite B34 V2 minute/intraday semantics audit. "
            "This is read-only with respect to providers and brokers."
        )
    )
    parser.add_argument(
        "--project-root",
    ...
    report = run_b34_audit(args.project_root)
    if not args.no_write:
        path = write_b34_report(args.project_root, report)
