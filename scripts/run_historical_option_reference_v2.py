...
from packages.data.historical_option_reference_v2 import (
    run_historical_option_reference_v2_acquisition,
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Acquire Historical Option Reference V2 with provider correction-lineage "
            "resolution and structural-reference authority only."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print("ATLAS Historical Option Reference V2 Acquisition")
    print(
        "  contract fingerprint: "
        f"{HISTORICAL_OPTION_REFERENCE_V2_CONTRACT_FINGERPRINT}"
    )
    ...
    summary = run_historical_option_reference_v2_acquisition(
        settings,
