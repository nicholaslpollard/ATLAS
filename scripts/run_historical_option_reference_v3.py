...
from packages.data.historical_option_reference_v3 import (
    run_historical_option_reference_v3_acquisition,
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Acquire Historical Option Reference V3 with narrow pre-expiration "
            "same-rank conflict resolution and structural-reference authority only."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print("ATLAS Historical Option Reference V3 Acquisition")
    print(
        "  contract fingerprint: "
        f"{HISTORICAL_OPTION_REFERENCE_V3_CONTRACT_FINGERPRINT}"
    )
    ...
    print(
        ...
        "exact-current-row match; otherwise fail closed"
    )
    print(
    ...
    summary = run_historical_option_reference_v3_acquisition(
        settings,
    ...
    print(
        "  V3 reused / V2 raw rebuilt / V1 raw rebuilt / provider acquired: "
