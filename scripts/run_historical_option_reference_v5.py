...
from packages.data.historical_option_reference_v5 import (
    run_historical_option_reference_v5_acquisition,
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Acquire Historical Option Reference V5 with point-in-time list+overview "
            "resolution for frozen unversioned identity and explicit-correction "
            "deliverable conflicts; structural-reference authority only."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print("ATLAS Historical Option Reference V5 Acquisition")
    print(
        "  contract fingerprint: "
        f"{HISTORICAL_OPTION_REFERENCE_V5_CONTRACT_FINGERPRINT}"
    )
    ...
    print("  verified V4/V3/V2/V1 raw reuse: enabled")
    ...
    summary = run_historical_option_reference_v5_acquisition(
        settings,
        workers=args.workers,
    )

    storage = summary["storage_after"]
    print("\nHISTORICAL OPTION REFERENCE V5: COMPLETE")
    ...
    print(
        "  V5 reused / V4 raw rebuilt / V3 raw rebuilt / V2 raw rebuilt / V1 raw rebuilt / provider acquired: "
