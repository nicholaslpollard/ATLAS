...
from packages.data.historical_option_reference_v6 import (
    run_historical_option_reference_v6_acquisition,
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Acquire Historical Option Reference V6 with frozen point-in-time "
            "conflict resolution plus auditable no-guess ambiguity quarantine; "
            "structural-reference authority only."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print("ATLAS Historical Option Reference V6 Acquisition")
    print(
        "  contract fingerprint: "
        f"{HISTORICAL_OPTION_REFERENCE_V6_CONTRACT_FINGERPRINT}"
    )
    ...
    print("  verified V5/V4/V3/V2/V1 raw reuse: enabled")
    print(f"  duplicate-version policy: {CORRECTION_SELECTION_POLICY}")
    print(
        "  same-rank conflict fallback: frozen historical exact-match branches; "
        "otherwise fail closed unless the narrow V6 ambiguity quarantine applies"
    )
    print(
    ...
    summary = run_historical_option_reference_v6_acquisition(
        settings,
    ...
    print(
        "  V6 reused / V5 raw rebuilt / V4 raw rebuilt / V3 raw rebuilt / V2 raw rebuilt / V1 raw rebuilt / provider acquired: "
