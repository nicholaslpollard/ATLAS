...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Retry only the accepted LIT-02 v1 source-unresolved cases with the predeclared "
            "official-SEC contextual repair-v2 rules. No market-price/return outcomes are read."
        )
    )
    parser.add_argument(
        "--acquire",
        action="store_true",
        help="Explicitly permit source/identity/transaction metadata reads for repair-v2.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Rebuild repair-v2 case checkpoints instead of reusing valid repair-v2 manifests.",
    )
    return parser
...
def main() -> int:
    ...
    if not args.acquire:
        print("ATLAS LIT-02 source metadata repair-v2 NOT STARTED.")
    ...
    print(f"  resolved cases after repair-v2:      {report['resolved_cases']}")
    print(f"  newly resolved cases:                {report['newly_resolved_cases']}")
    print(f"  unresolved cases after repair-v2:    {report['unresolved_cases']}")
    ...
    print(f"  SEC lookback days:                   {report['repair_v2_sec_lookback_days']}")
    print(f"  SEC allowed forms:                   {json.dumps(report['repair_v2_sec_allowed_forms'])}")
    ...
    print(f"  repair-v2 cached cases reused:       {report['cached_case_manifests_reused']}")
    print(f"  v1 resolved cases reused:            {report['v1_resolved_cases_reused']}")
    print(f"  v1 unresolved cases retried:         {report['v1_unresolved_cases_retried']}")
