...
def _print_preparation(preparation) -> None:
    ...
    print(f"Selected PAPER read context: {preparation.broker.value}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "execution may perform only exact run-scoped external reads and never order mutations."
        )
    )
    ...
    parser.add_argument(
        ...
        help="PAPER broker context for future read-only portfolio evidence. Webull is default.",
    )
    args = parser.parse_args()

    cycle = Phase23CurrentAnalysisCycle(load_settings())
    try:
    ...
    except (Phase23CurrentRunError, Phase23AuthorizationError, ValueError) as exc:
        print("Execution status: BLOCKED")
        print(f"Reason: {exc}")
