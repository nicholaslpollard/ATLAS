...
def test_recurrent_empty_cycle_completes_without_account_mutation(
    tmp_path,
) -> None:
    checkpoint_path = tmp_path / "current.json"
    runtime, _result = bootstrap_recurrent_genesis_v1(
    ...
    path, receipt = begin_recurrent_cycle(
        checkpoint_path=checkpoint_path,
        runtime=runtime,
        cycle_id="empty-smoke-2026-09-18T20:05:00Z",
        now_utc=datetime(2026, 9, 18, 20, 5, tzinfo=UTC),
    )
    ...
    assert completed.status == RecurrentCycleStatus.COMPLETE
    assert tuple(stage.stage for stage in completed.stages) == (
        RecurrentCycleStage.CLOSE,
        RecurrentCycleStage.RESERVE,
        RecurrentCycleStage.ENTRY,
        RecurrentCycleStage.MARK,
    )
    assert all(stage.action_fingerprints == () for stage in completed.stages)
