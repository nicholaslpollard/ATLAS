...
def test_spy_benchmark_uses_exact_final_regular_minute() -> None:
    sessions = (runner.DEVELOPMENT_START, runner.DEVELOPMENT_START + timedelta(days=1))
    source = _FakeMinuteSource(_spy_frame(sessions), sessions)
    ...
    assert benchmark["close"].tolist() == [501.0, 502.0]
...
def test_spy_benchmark_duplicate_minute_key_fails_closed() -> None:
    sessions = (runner.DEVELOPMENT_START,)
    frame = _spy_frame(sessions)
    ...
    with pytest.raises(runner.SuccessorDevelopmentRunnerError, match="duplicate minute keys"):
        runner.build_spy_benchmark_from_accepted_minute_source(source, plan)
